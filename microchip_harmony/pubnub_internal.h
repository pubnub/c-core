...
typedef NET_PRES_SKT_HANDLE_T pb_socket_t;
        ...
        #define socket_send(socket, buf, len) \
            NET_PRES_SocketWrite((socket), (buf), (len))
        #define socket_recv(socket, buf, len) \
            NET_PRES_SocketRead((socket), (buf), (len))
...
typedef TCP_SOCKET pb_socket_t;
        ...
        #define socket_send(socket, buf, len) \
            TCPIP_TCP_ArrayPut((socket), (buf), (len))
        #define socket_recv(socket, buf, len) \
            TCPIP_TCP_ArrayGet((socket), (buf), (len))
