...
void pubnub_task(void)
{
    ...
    if (0 == s_tick_prev) { s_tick_prev = SYS_TMR_TickCountGet(); }
    ...
    if (m_watcher.apb_size > 0) {
        ...
        for (ppbp = m_watcher.apb; ppbp < m_watcher.apb + m_watcher.apb_size;
             ++ppbp) {
            pbnc_fsm(*ppbp);
        }
        if (elapsed > 0) {
    ...
    if (i == m_watcher.apb_size) {
        ...
        if (PUBNUB_TIMERS_API) {
            m_watcher.timer_head =
                pubnub_timer_list_add(m_watcher.timer_head, pb);
        }
    }
    return +1;
}
