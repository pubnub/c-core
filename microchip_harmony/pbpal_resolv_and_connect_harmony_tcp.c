...
enum pbpal_resolv_n_connect_result pbpal_resolv_and_connect(pubnub_t* pb) {
    ...
    PUBNUB_ASSERT(
        (pb->state == PBS_IDLE) || (pb->state == PBS_WAIT_DNS_SEND)
...
pbpal_check_resolv_and_connect(pubnub_t* pb) {
    ...
    TCPIP_DNS_RESULT dns_result =
        TCPIP_DNS_IsResolved(origin, &ip_addr, IP_ADDRESS_TYPE_IPV4);
        ...
        case TCPIP_DNS_RES_OK:
            if (SOCKET_INVALID == pb->pal.socket) {
                ...
                pb->pal.socket = NET_PRES_SocketOpen(
                    0,
                    pb->options.useSSL ? NET_PRES_SKT_ENCRYPTED_STREAM_CLIENT
                                       : NET_PRES_SKT_UNENCRYPTED_STREAM_CLIENT,
                    IP_ADDRESS_TYPE_IPV4,
                    HTTPS_PORT,
                    (NET_PRES_ADDRESS*)&ip_addr,
                    NULL);
#else
                pb->pal.socket = TCPIP_TCP_ClientOpen(
                    IP_ADDRESS_TYPE_IPV4,
                    HTTP_PORT,
                    &ip_addr);
#endif
             ...
             * `NET_PRES_` functions...
             */
            return socket_connect(pb->pal.socket) ? pbpal_connect_failed
...
#else
    return TCPIP_TCP_IsConnected(pb->pal.socket) ? pbpal_connect_success
                                                 : pbpal_connect_wouldblock;
#endif
}
