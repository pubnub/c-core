...
static void PubnubCallbackSample(
    ...
    switch (pData->state) {
        case sastInit:
            if (pbp == NULL) {
                ...
                pubnub_set_transaction_timeout(
                    pbp,
                    PUBNUB_DEFAULT_NON_SUBSCRIBE_TIMEOUT);

                res =
                    pubnub_register_callback(pbp, PubnubCallbackSample, pData);
                if (PNR_OK != res) {
                    PUBNUB_LOG_INFO(
                        "Failed to register callback, error: %d\r\n",
                        res);
                    pubnub_free(pbp);
                    return;
                }
                ...
                res = pubnub_publish(
                    pbp,
                    chan,
                    "\"Hello world from MPLAB Harmony callback!\"");
                if (res != PNR_STARTED) {
                    PUBNUB_LOG_INFO(
                        "pubnub_publish() returned unexpected: %d\r\n",
                        res);
                    pubnub_free(pbp);
                    return;
                }
        ...
        case sastPublish:
            ...
            if (res == PNR_STARTED) {
                PUBNUB_LOG_INFO(
                    "Publish outcome unexpected: PNR_STARTED(%d)\r\n",
                    res);
                pubnub_free(pbp);
            ...
            if (res != PNR_STARTED) {
                PUBNUB_LOG_INFO(
                    "pubnub_subscribe() returned unexpected: %d\r\n",
                    res);
                pubnub_free(pbp);
                pData->state = sastInit;
                return;
        ...
        case sastFirstSubscribe:
            ...
            if (res == PNR_STARTED) {
                PUBNUB_LOG_INFO(
                    "Subscribe outcome unexpected: PNR_STARTED(%d)\r\n",
                    res);
                pubnub_free(pbp);
                pData->state = sastInit;
                return;
        ...
        case sastSubscribe:
            ...
            if (res != PNR_STARTED) {
                PUBNUB_LOG_INFO(
                    "pubnub_time() returned unexpected: %d\r\n",
                    res);
                pubnub_free(pbp);
                pData->state = sastInit;
                return;
        ...
        case sastTime:
            ...
            if (res == PNR_STARTED) {
                PUBNUB_LOG_INFO(
                    "Time outcome unexpected: PNR_STARTED(%d)\r\n",
                    res);
                pubnub_free(pbp);
                pData->state = sastInit;
                return;
