...
static void generate_uuid(pubnub_t* pbp)
{
    ...
    if (0 != pubnub_generate_uuid_v4_random(&uuid)) {
        ...
        PUBNUB_LOG_DEBUG(pbp, "Generated UUID: %s", str_uuid.uuid);
    }
}
...
static void PubnubCallbackSample(
    pubnub_t*         pbp,
    ...
    void*             user_data)
{
    ...
    case sastInit:
        if (pbp == NULL) {
            ...
            pubnub_set_transaction_timeout(
                pbp, PUBNUB_DEFAULT_NON_SUBSCRIBE_TIMEOUT);

            res = pubnub_register_callback(pbp, PubnubCallbackSample, pData);
            if (PNR_OK != res) {
#if PUBNUB_LOG_ENABLED(ERROR)
                if (pubnub_logger_should_log(pbp, PUBNUB_LOG_LEVEL_ERROR)) {
                    pubnub_log_error(
                        pbp,
                        PUBNUB_LOG_LOCATION,
                        res,
            ...
            PUBNUB_LOG_DEBUG(pbp, "Publishing...");
            res = pubnub_publish(
                pbp, chan, "\"Hello world from MPLAB Harmony callback!\"");
            if (res != PNR_STARTED) {
#if PUBNUB_LOG_ENABLED(ERROR)
                if (pubnub_logger_should_log(pbp, PUBNUB_LOG_LEVEL_ERROR)) {
                    pubnub_log_error(
                        ...
                        PUBNUB_LOG_LOCATION,
                        res,
                        "Publish returned unexpected result",
                        pubnub_res_2_string(res));
                }
#endif // PUBNUB_LOG_ENABLED(ERROR)
                pubnub_free(pbp);
            ...
            PUBNUB_LOG_ERROR(pbp, "Received unexpected outcome: %d", res);
            pubnub_free(pbp);
            return;
        }
        break;
    case sastPublish:
        ...
        PUBNUB_LOG_INFO(pbp, "Subscribing...");
    ...
    case sastFirstSubscribe:
        ...
        if (PNR_OK == res) { PUBNUB_LOG_INFO(pbp, "Subscribed!"); }
        else {
#if PUBNUB_LOG_ENABLED(ERROR)
            if (pubnub_logger_should_log(pbp, PUBNUB_LOG_LEVEL_ERROR)) {
                pubnub_log_error(
    ...
    case sastSubscribe:
        ...
        if (PNR_OK == res) {
            PUBNUB_LOG_DEBUG(pbp, "Subscribed! Got messages:");
            for (;;) {
                msg = pubnub_get(pbp);
                if (NULL == msg) { break; }
                PUBNUB_LOG_DEBUG(pbp, "%s", msg);
            }
        }
        else {
#if PUBNUB_LOG_ENABLED(ERROR)
        ...
        PUBNUB_LOG_INFO(pbp, "Getting time...");
        res = pubnub_time(pbp);
        if (res != PNR_STARTED) {
#if PUBNUB_LOG_ENABLED(ERROR)
            if (pubnub_logger_should_log(pbp, PUBNUB_LOG_LEVEL_ERROR)) {
                pubnub_log_error(
                    ...
                    PUBNUB_LOG_LOCATION,
                    res,
                    "Time returned unexpected result",
                    pubnub_res_2_string(res));
            }
#endif // PUBNUB_LOG_ENABLED(ERROR)
            pubnub_free(pbp);
    ...
    case sastTime:
        ...
        if (PNR_OK == res) {
            ...
            if (pubnub_logger_should_log(pbp, PUBNUB_LOG_LEVEL_DEBUG)) {
                ...
                PUBNUB_LOG_MAP_SET_STRING(&data, pubnub_get(pbp), timetoken)
                PUBNUB_LOG_MAP_SET_STRING(
                    &data, pubnub_last_time_token(pbp), last_timetoken)
                pubnub_log_object(
                    pbp,
                    PUBNUB_LOG_LEVEL_DEBUG,
                    PUBNUB_LOG_LOCATION,
                    &data,
                    "Received time response:");
            }
        ...
        if (pubnub_free(pbp) != 0) {
            PUBNUB_LOG_ERROR(pbp, "Failed to free the Pubnub context.");
        }
        printf("Pubnub FreeRTOS callback demo over.\r\n");
        pData->state = sastInit;
        break;
    }
}
        ...
        for (i = 0; i < nNets; i++) {
            ...
            if (0 != ipAddr.Val) {
                ...
                SYS_CONSOLE_PRINT(
                    ...
                    ipAddr.v[3]);
                if (ipAddr.v[0] != 0 &&
                    ipAddr.v[0] != 169) // Wait for a Valid IP
                {
