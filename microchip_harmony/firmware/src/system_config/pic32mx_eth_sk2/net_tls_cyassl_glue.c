...
int NET_TLS_CG_Stream_Send_cb(void* sslin, char* buf, int sz, void* ctx) {
    ...
    bufferSize = (*spTransData->fpStreamWrite)(
        (uintptr_t)fd,
        (uint8_t*)buf,
        (uint16_t)sz);
    return bufferSize;
...
bool NET_TLS_CyaSSLStreamClientCreate(
    ...
    int8_t certIndex,
    void* providerData) {
    NET_TLS_CyaSSLGlueProvData* pData =
        (NET_TLS_CyaSSLGlueProvData*)providerData;
    ...
    if (CyaSSL_CTX_load_verify_buffer(
            pData->ctx,
            caCert,
            caCert_len,
            SSL_FILETYPE_PEM)
...
int32_t
NET_TLS_CyaSSLWrite(void* providerData, const uint8_t* buffer, uint16_t size) {
    NET_TLS_CyaSSLGlueProvData* pData =
        (NET_TLS_CyaSSLGlueProvData*)providerData;
