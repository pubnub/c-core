 ...
 */
        ...
        #define PUBNUB_ASSERT_UINT_EX(a, op, b) \
            PUBNUB_ASSERT_UINT_IMPL(a, op, b)
    #else
        #define PUBNUB_ASSERT_UINT_EX(a, op, b) \
            PUBNUB_UNUSED(a); \
            PUBNUB_UNUSED(b)
 ...
 */
void PUBNUB_NORETURN
pubnub_assert_failed(char const* s, char const* file, long line);
 ...
 */
typedef void PUBNUB_NORETURN (
    *pubnub_assert_handler_t)(char const* s, char const* file, long line);
...
*/
void PUBNUB_NORETURN
pubnub_assert_handler_loop(char const* s, char const* file, long line);

/** This handler will print a message  formed from the parameters and
    then abort (exit, end) the process. Useful for testing.
 */
void PUBNUB_NORETURN
pubnub_assert_handler_abort(char const* s, char const* file, long line);
 ...
 */
        #define PUBNUB_STATIC_ASSERT(cond, msg) \
            extern struct PUBNUB_CTASRT(static_assert, _failed_, msg) { \
                int PUBNUB_CTASRT(static_assert, _failed_, msg) : !!(cond); \
