...
pubnub_t*
pubnub_timer_list_add(pubnub_t* list, pubnub_t* to_add, int timeout_to_add_ms) {
    pubnub_t* pbp;
    ...
    if (NULL == list) {
        PUBNUB_LOG_TRACE(
            "pubnub_timer_list_add(list=NULL, to_add=%p, timeout_to_add_ms=%d)\n",
            to_add,
            timeout_to_add_ms);
        list = to_add;
    ...
    if (timeout_to_add_ms < list->timeout_left_ms) {
        PUBNUB_LOG_TRACE(
            "pubnub_timer_list_add(list=%p, to_add=%p, timeout_to_add_ms=%d): "
    ...
    while (timeout_to_add_ms >= pbp->timeout_left_ms) {
        PUBNUB_LOG_TRACE(
            "pubnub_timer_list_add(list=%p, to_add=%p, timeout_to_add_ms=%d) while: "
        ...
        if (NULL == pbp->next) {
            PUBNUB_LOG_TRACE(
                "pubnub_timer_list_add() end while: "
    ...
    PUBNUB_LOG_TRACE(
        "pubnub_timer_list_add() crocodile: "
...
pubnub_t* pubnub_timer_list_remove(pubnub_t* list, pubnub_t* to_remove) {
    ...
    if (list == to_remove) {
        PUBNUB_LOG_TRACE(
            "pubnub_timer_list_remove(list=%p == to_remove): "
            "list->timeout_left_ms=%d\n",
            list,
            list->timeout_left_ms);
        ...
        if (NULL != list) {
            PUBNUB_LOG_TRACE(
                "pubnub_timer_list_remove() has next: "
    ...
    if (NULL == to_remove->next) {
        PUBNUB_LOG_TRACE(
            "pubnub_timer_list_remove(list=%p, to_remove=%p) tail\n",
            list,
            to_remove);
    ...
    PUBNUB_LOG_TRACE(
        "pubnub_timer_list_remove(list=%p, to_remove=%p) default:\n"
...
pubnub_t*
pubnub_timer_list_as_time_goes_by(pubnub_t** pplist, int time_passed_ms) {
    pubnub_t* list;
