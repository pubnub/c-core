    ...
    switch (status) {
        case TINFL_STATUS_DONE:
            if (dst_buf_size == out_len) {
                ...
                PUBNUB_LOG_ERROR(
                    "Decompressed length[%u] is smaller than the "
        ...
        case TINFL_STATUS_HAS_MORE_OUTPUT:
            PUBNUB_LOG_ERROR(
                "Decompressed length is grater than the "
                "'unpacked_size' value[%u]!\n"
                "(Unpacked:['%.*s'])\n",
                (unsigned)out_len,
                (int)out_len,
...
static enum pubnub_res inflate_total(
    ...
    if (pb->core.decomp_buf_size < out_len) {
        ...
        if (NULL == newbuf) {
            PUBNUB_LOG_ERROR(
                "Failed to reallocate decompression buffer!\n"
                "Out length:%lu\n",
...
enum pubnub_res pbgzip_decompress(pubnub_t* pb) {
    ...
    if (data[2] != 8) {
        PUBNUB_LOG_ERROR(
            "Unknown compression method %uX - only 'deflate'(8) "
            "is supported!\n",
            (unsigned)data[2]);
        return PNR_BAD_COMPRESSION_FORMAT;
    }
    if (data[3] != 0) {
        PUBNUB_LOG_ERROR(
            "GZIP flags should be 0, but are %uX\n",
            (unsigned)data[3]);
        return PNR_BAD_COMPRESSION_FORMAT;
    }
    ...
    PUBNUB_LOG_TRACE(
        "pbgzip_decompress(pb=%p)-Length before:%lu and after "
        "decompresion:%lu\n",
        pb,
        (unsigned long)size,
