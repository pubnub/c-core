...
enum pubnub_res pbpal_line_read_status(pubnub_t* pb)
{
    ...
    if (pb->unreadlen == 0) {
        ...
        if (recvres < 0) { return PNR_IN_PROGRESS; }
        else if (0 == recvres) {
    ...
    if (pb->left == 0) {
        PUBNUB_LOG_ERROR(pb, "Buffer full but newline not found.");
        pb->sock_state = STATE_NONE;
...
enum pubnub_res pbpal_read_status(pubnub_t* pb)
{
    ...
    if (0 == pb->unreadlen) {
        ...
        if (have_read < 0) { return PNR_IN_PROGRESS; }
        else if (0 == have_read) {
            pb->sock_state = STATE_NONE;
            return PNR_TIMEOUT;
        }
...
{
    //    expect(pbntf_enqueue_for_processing, when(pb, equals(pbp)),
    //    returns(0));
    expect(
        pbpal_resolv_and_connect,
        when(pb, equals(pbp)),
...
static inline void expect_outgoing_with_encoded_credentials_CONNECT(
    char const* url,
    char const* HTTP_proxy_header)
{
    expect(pbpal_send, when(data, streqs("CONNECT ")), returns(0));
...
static inline void incoming(char const* str)
{
    ...
    if (m_num_msgrcvd == 1) { m_read = pmsg; }
}
...
Ensure(single_context_pubnub, establishes_proxy_connection_GET_Basic)
{
    ...
    attest(
        pubnub_set_proxy_manual(pbp, 5, "localhost", proxy_port), equals(-1));
    /* proxy hostname/url too long */
    attest(
        pubnub_set_proxy_manual(
            pbp, pbproxyHTTP_GET, proxy_hostname_to_long, proxy_port),
        equals(-1));
    /* parameters acceptable */
    attest(
        pubnub_set_proxy_manual(
            pbp, pbproxyHTTP_GET, "proxy_server_url", proxy_port),
        equals(0));
    attest(
        pubnub_set_proxy_authentication_username_password(
            pbp, "some_user", "some_password"),
        equals(0));
    expect_have_dns_for_proxy_server();
    expect_first_outgoing_GET(
        "/subscribe/sub-key/colors/0/0?pnsdk=unit-test-0.1&uuid=test_id");
    incoming(
...
Ensure(
    ...
    proxy_GET_Basic_client_sets_timeout_and_max_operation_count_for_keep_alive)
{
    uint16_t proxy_port = 500;
    time_t   await_time;
    pubnub_init(pbp, "publ-key", "sub-key");
    ...
    attest(
        pubnub_set_proxy_manual(
            pbp, pbproxyHTTP_GET, "46.235.225.189", proxy_port),
        equals(0));
    attest(
        pubnub_set_proxy_authentication_username_password(
            pbp, "some_user", "some_password"),
        equals(0));
    /* Transactions' limits: up to whole second, 4 operations until client's
     * connection closure */
    ...
    expect(pbntf_got_socket, when(pb, equals(pbp)), returns(0));
    expect_outgoing_with_encoded_credentials_GET(
        "/publish/publ-key/sub-key/0/jarak/0/"
        "%22zec%22?pnsdk=unit-test-0.1&uuid=test_id",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming(
        "HTTP/1.1 200\r\nContent-Length: "
    ...
    //    expect_have_dns_for_proxy_server();
    expect_outgoing_with_encoded_credentials_GET(
        "/subscribe/sub-key/jarak/0/"
        "1516714978925123457?pnsdk=unit-test-0.1&uuid=test_id",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming_and_close(
        "HTTP/1.1 200\r\n"
        "Server: proxy_server.com\r\n"
    ...
    expect_have_dns_for_proxy_server();
    expect_outgoing_with_encoded_credentials_GET(
        "/v2/history/sub-key/sub-key/channel/"
        "ch?pnsdk=unit-test-0.1&uuid=test_id&count=22&include_token=true",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming(
        "HTTP/1.1 200\r\nContent-Length: "
    ...
    attest(
        pubnub_get(pbp),
        streqs(
            "[{\"message\":1,\"timetoken\":14370863460777883},{\"message\":2,"
            "\"timetoken\":14370863461279046},{\"message\":3,\"timetoken\":"
            "14370863958459501}]"));
...
Ensure(
    single_context_pubnub,
    GET_Basic_proxy_closes_connection_after_first_407answer_client_keeps_folloving_protocol_until_established)
{
    uint16_t proxy_port = 500;
    pubnub_init(pbp, "publ-key", "sub-key");
    srand(1);
    ...
    attest(
        pubnub_set_proxy_manual(
            pbp, pbproxyHTTP_GET, "2a00:1098::82:1000:3b:1:1", proxy_port),
        equals(0));
    attest(
        pubnub_set_proxy_authentication_username_password(
            pbp, "some_user", "some_password"),
    ...
    incoming_and_close(
        ...
        "</html>\n");
    attest(
        pubnub_subscribe(pbp, "colors", NULL),
        equals(PNR_AUTHENTICATION_FAILED));
...
Ensure(
    single_context_pubnub,
    establishes_proxy_connection_GET_Digest_and_continues_negotiating_after_stale_nounce)
{
    uint16_t port = 500;
#if __APPLE__
    char const*
        HTTP_proxy_header1 = "\r\nProxy-Authorization: Digest "
                             ...
                             "uri=\"/subscribe/sub-key/music/0/"
                             "0?pnsdk=unit-test-0.1&uuid=test_id\", "
    ...
    char const*
        HTTP_proxy_header2 = "\r\nProxy-Authorization: Digest "
                             ...
                             "nonce=\"dcd98b7102dd2f0e8b11d0f600bfb0c093\", "
                             "uri=\"/subscribe/sub-key/music/0/"
                             "1516014978925123457?pnsdk=unit-test-0.1&uuid="
                             "test_id\", "
                             "cnonce=\"a7410000f13a461099acb7602a0cb53a\", "
                             "nc=\"00000002\", "
                             "qop=\"auth-int\", "
    ...
    char const*
        HTTP_proxy_header3 = "\r\nProxy-Authorization: Digest "
                             ...
                             "nonce=\"dcd98b7102dd2f0e8b11d0f600bfb0c093\", "
                             "uri=\"/subscribe/sub-key/music/0/"
                             "1516014978925123557?pnsdk=unit-test-0.1&uuid="
                             "test_id\", "
                             "cnonce=\"a7410000f13a461099acb7602a0cb53a\", "
                             "nc=\"00000003\", "
                             "qop=\"auth-int\", "
    ...
    attest(
        pubnub_set_proxy_manual(
            pbp, pbproxyHTTP_GET, "proxy.mythic-beasts.com", port),
        equals(0));
    attest(
        pubnub_set_proxy_authentication_username_password(
            pbp, "average_user", "password"),
        equals(0));
    expect_have_dns_for_proxy_server();
    expect_first_outgoing_GET(
    ...
    expect_outgoing_with_encoded_credentials_GET(
        "/subscribe/sub-key/music/0/"
        "1516014978925123557?pnsdk=unit-test-0.1&uuid=test_id",
        HTTP_proxy_header3);
    incoming(
        "HTTP/1.0 407 ProxyAuthentication Required\r\n"
        "Server: proxy_server.com\r\n"
...
Ensure(
    single_context_pubnub,
    GET_Digest_proxy_closes_connection_after407_and_stale_nounce_but_keeps_negotiating)
{
    uint16_t port = 500;
#if __APPLE__
    char const*
    ...
    char const*
        HTTP_proxy_header5 = "\r\nProxy-Authorization: Digest "
                             ...
                             "nonce=\"dcd98b7102cc2f0e8b11d0f700bfb0c093\", "
                             "uri=\"/subscribe/sub-key/music/0/"
                             "1516014978925123577?pnsdk=unit-test-0.1&uuid="
                             "test_id\", "
                             "cnonce=\"82b73144c8da461c988e0506fe09e556\", "
                             "nc=\"00000002\", "
                             "qop=\"auth-int\", "
    ...
    attest(
        pubnub_set_proxy_manual(pbp, pbproxyHTTP_GET, "proxy_server_url", port),
        equals(0));
    attest(
        pubnub_set_proxy_authentication_username_password(
            pbp, "average_user", "password"),
    ...
    /* Digest sheme - Sending repeated 'authentication required' messages on
       same realm within the same transaction should be considered eroneous
     */
    expect(pbntf_enqueue_for_processing, when(pb, equals(pbp)), returns(0));
    expect(pbntf_got_socket, when(pb, equals(pbp)), returns(0));
    expect_outgoing_with_encoded_credentials_GET(
        "/subscribe/sub-key/music/0/"
        "1516014978925123577?pnsdk=unit-test-0.1&uuid=test_id",
        HTTP_proxy_header5);
    incoming(
        "HTTP/1.0 407 ProxyAuthentication Required\r\n"
        "Server: proxy_server.com\r\n"
...
Ensure(single_context_pubnub, try_to_establish_proxy_connection_GET_No_response)
{
    ...
    expect_first_outgoing_GET(
        "/subscribe/sub-key/music/0/0?pnsdk=unit-test-0.1&uuid=test_id");
    /* First incoming string is empty - 'resv' returns 0 - server closed
       connection from some reason. (Assumed connection timeout) */
    incoming_and_close("");
...
Ensure(
    single_context_pubnub,
    try_to_establish_proxy_connection_GET_unsupported_proxy_authentication)
{
    uint16_t port = 500;
    pubnub_init(pbp, "publ-key", "sub-key");
    srand(1);
    ...
    expect_first_outgoing_GET(
        "/subscribe/sub-key/music/0/0?pnsdk=unit-test-0.1&uuid=test_id");
    incoming_and_close(
        "HTTP/1.1 407 ProxyAuthentication Required ( The ISA Server requires "
...
Ensure(single_context_pubnub, establishes_proxy_connection_CONNECT_Basic)
{
    ...
    attest(
        pubnub_set_proxy_manual(
            pbp, pbproxyHTTP_CONNECT, "proxy_server_url", proxy_port),
        equals(0));
    attest(
        pubnub_set_proxy_authentication_username_password(
            pbp, "serious", "password"),
        equals(0));
...
Ensure(single_context_pubnub, establishes_proxy_connection_CONNECT_Digest)
{
    ...
    char const*
        HTTP_proxy_header = "\r\nProxy-Authorization: Digest "
                            ...
                            "realm=\"testrealm@host.com\", "
                            "nonce=\"dcd98b7102dd2f0e8b11d0f600bfb0c093\", "
                            "uri=\"/subscribe/sub-key/health/0/"
                            "0?pnsdk=unit-test-0.1&uuid=test_id\", "
                            "cnonce=\"a7410000f13a461099acb7602a0cb53a\", "
                            "nc=\"00000001\", "
                            "qop=\"auth-int\", "
    ...
    attest(
        pubnub_set_proxy_manual(pbp, pbproxyHTTP_CONNECT, "HTTPd/0.9", port),
        equals(0));
    attest(
        pubnub_set_proxy_authentication_username_password(
            pbp, "serious", "password"),
...
{
    expect_assert_in(
        pubnub_set_proxy_manual(NULL, pbproxyHTTP_GET, "proxy_server_url", 500),
        "pubnub_proxy.c");
    expect_assert_in(
        pubnub_set_proxy_manual(pbp, pbproxyHTTP_GET, NULL, 500),
        "pubnub_proxy.c");
