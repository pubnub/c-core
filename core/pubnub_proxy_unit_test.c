...
Ensure(single_context_pubnub, establishes_proxy_connection_GET_Basic) {
    ...
    attest(
        pubnub_set_proxy_manual(pbp, 5, "localhost", proxy_port),
        equals(-1));
    ...
    expect_outgoing_with_encoded_credentials_GET(
        "/subscribe/sub-key/colors/0/1516014978925123457?pnsdk=unit-test-0.1&uuid=test_id",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming(
        "HTTP/1.1 200\r\n"
        "Server: proxy_server.com\r\n"
...
Ensure(
    ...
    expect(pbntf_got_socket, when(pb, equals(pbp)), returns(0));
    expect_outgoing_with_encoded_credentials_GET(
        "/publish/publ-key/sub-key/0/jarak/0/%22zec%22?pnsdk=unit-test-0.1&uuid=test_id",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming(
    ...
    //    expect_have_dns_for_proxy_server();
    expect_outgoing_with_encoded_credentials_GET(
        "/subscribe/sub-key/jarak/0/1516714978925123457?pnsdk=unit-test-0.1&uuid=test_id",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming_and_close(
        "HTTP/1.1 200\r\n"
        "Server: proxy_server.com\r\n"
    ...
    expect_have_dns_for_proxy_server();
    expect_outgoing_with_encoded_credentials_GET(
        "/v2/history/sub-key/sub-key/channel/ch?pnsdk=unit-test-0.1&uuid=test_id&count=22&include_token=true",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming(
    ...
    attest(
        pubnub_get(pbp),
        streqs(
            "[{\"message\":1,\"timetoken\":14370863460777883},{\"message\":2,\"timetoken\":14370863461279046},{\"message\":3,\"timetoken\":14370863958459501}]"));
...
Ensure(
    ...
    expect(pbntf_got_socket, when(pb, equals(pbp)), returns(0));
    expect_outgoing_with_encoded_credentials_GET(
        "/subscribe/sub-key/colors/0/1516714978925123457?pnsdk=unit-test-0.1&uuid=test_id",
        "\r\nProxy-Authorization: Basic c29tZV91c2VyOnNvbWVfcGFzc3dvcmQ=");
    incoming_and_close(
        "HTTP/1.1 407 ProxyAuthentication Required\r\n"
        "Server: proxy_server.com\r\n"
...
Ensure(
    single_context_pubnub,
    try_to_establish_proxy_connection_GET_No_response) {
    uint16_t port = 500;
    pubnub_init(pbp, "publ-key", "sub-key");
    pubnub_set_user_id(pbp, "test_id");
...
Ensure(
    ...
    expect_first_outgoing_GET(
        "/subscribe/sub-key/music/0/0?pnsdk=unit-test-0.1&uuid=test_id");
    incoming_and_close(
        "HTTP/1.1 407 ProxyAuthentication Required ( The ISA Server requires authorization to fulfill the request. Access to the Web proxy service is denied. )\r\n"
        "Via: 1.1 SPIRIT1B\r\n"
...
Ensure(single_context_pubnub, establishes_proxy_connection_CONNECT_Basic) {
    ...
    expect(pbntf_got_socket, when(pb, equals(pbp)), returns(0));
    expect_outgoing_with_url(
        "/subscribe/sub-key/health/0/1516014978925123457?pnsdk=unit-test-0.1&uuid=test_id");
    incoming(
