...
pubnub_chamebl_t pbcc_get_grant_token(struct pbcc_context* pb) {
    ...
    if (pb->last_result != PNR_OK) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_grant_token(pb=%p) can be called only if "
    ...
    if (json_rslt != jonmpOK) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_grant_token(pbcc=%p) - Invalid response: "
...
enum pubnub_res pbcc_parse_grant_token_api_response(struct pbcc_context* pb) {
    ...
    if ((*reply != '{') || (*(elem.end++) != '}')) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_grant_token_api_response(pbcc=%p) - Invalid: "
    ...
    if (pbjson_value_for_field_found(&elem, "status", "403")) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_grant_token_api_response(pbcc=%p) - AccessDenied: "
    ...
    if (jonmpKeyNotFound == json_rslt) {
        ...
        if (json_rslt != jonmpOK) {
            PUBNUB_LOG_ERROR(
                "pbcc_parse_grant_token_api_response(pbcc=%p) - Invalid response: "
        ...
        PUBNUB_LOG_WARNING(
            "pbcc_parse_grant_token_api_response(pbcc=%p): \"error\"='%.*s'\n",
            pb,
    ...
    PUBNUB_LOG_TRACE(
        "pbcc_parse_grant_token_api_response(pbcc=%p): \"data\"='%.*s'\n",
        pb,
        (int)(parsed.end - parsed.start),
        parsed.start);
