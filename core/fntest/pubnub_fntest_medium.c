...
TEST_DEF_NEED_CHGROUP(
    ...
    expect_PNR_OK(
        pbp,
        pubnub_add_channel_to_group(pbp, "two,three", chgrp),
        10 * SECONDS);
...
TEST_DEF(connect_disconnect_and_connect_again) {
    ...
    await_timed(10 * SECONDS, PNR_CANCELLED, pbp);
    expect_pnr(
        pubnub_publish(pbp, this_test_name_, "\"Test M5 - 3\""),
        PNR_STARTED);
    await_timed(10 * SECONDS, PNR_OK, pbp);
...
TEST_DEF_NEED_CHGROUP(connect_disconnect_and_connect_again_group) {
    ...
    expect_pnr(pubnub_remove_channel_from_group(pbp, "ch", chgrp), PNR_STARTED);
    await_timed(10 * SECONDS, PNR_OK, pbp);
...
TEST_DEF_NEED_CHGROUP(connect_disconnect_and_connect_again_combo) {
    ...
    expect_pnr_maybe_started(rslt, pbp, 10 * SECONDS, PNR_OK);
    expect(
        pnfntst_got_messages(pbp, "\"Test M8 - 2\"", "\"Test M9 - 2\"", NULL));
    ...
    await_timed(10 * SECONDS, PNR_OK, pbp);
    expect(
        pnfntst_got_messages(pbp, "\"Test M8 - 3\"", "\"Test M9 - 3\"", NULL));
...
TEST_DEF(wrong_api_usage) {
    ...
    expect_pnr(pubnub_subscribe(pbp, this_test_name_, NULL), PNR_STARTED);
    expect_pnr(
        pubnub_publish(pbp, this_test_name_, "\"Test - 2\""),
        PNR_IN_PROGRESS);
