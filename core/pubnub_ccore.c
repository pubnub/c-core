...
static enum pubnub_res simple_parse_response(struct pbcc_context* p) {
    ...
    if (pbjson_value_for_field_found(&el, "status", "403")) {
        PUBNUB_LOG_ERROR(
            "simple_parse_response(pbcc=%p) - AccessDenied: "
...
enum pubnub_res pbcc_history_prep(
    ...
    if (count) {
        ADD_URL_PARAM(qparam, count, cnt_buf);
    }
...
enum pubnub_res pbcc_heartbeat_prep(
    ...
    if (pb->state) {
        ADD_URL_PARAM(qparam, state, pb->state);
    }
#if PUBNUB_CRYPTO_API
    if (pb->secret_key == NULL) {
        ADD_URL_AUTH_PARAM(pb, qparam, auth);
...
enum pubnub_res
pbcc_where_now_prep(struct pbcc_context* pb, const char* user_id) {
    PUBNUB_ASSERT_OPT(user_id != NULL);
    enum pubnub_res rslt = PNR_OK;
    char const* const uname = pubnub_uname();
    ...
    if (pb_user_id) {
        ADD_URL_PARAM(qparam, uuid, pb_user_id);
    }
#if PUBNUB_CRYPTO_API
    if (pb->secret_key == NULL) {
        ADD_URL_AUTH_PARAM(pb, qparam, auth);
...
#endif
    if (state) {
        ADD_URL_PARAM(qparam, state, state);
    }
...
#endif
    PUBNUB_LOG_DEBUG(
        "pbcc_remove_channel_group_prep. REQUEST =%s\n",
        pb->http_buf);
    return (rslt != PNR_OK) ? rslt : PNR_STARTED;
}
