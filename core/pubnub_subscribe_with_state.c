...
static enum pubnub_res pbcc_subscribe_with_state_prep(
    ...
    if (state) {
        APPEND_URL_PARAM(qparam, state, state);
    }
...
enum pubnub_res pubnub_subscribe_with_state(
    pubnub_t* p,
    const char* channel,
    const char* channel_group,
    char const* state) {
    ...
    rslt =
        pbcc_subscribe_with_state_prep(&p->core, channel, channel_group, state);
    if (PNR_STARTED == rslt) {
