...
static bool should_keep_alive(struct pubnub_* pb, enum pubnub_res rslt) {
    ...
    if (!pb->flags.should_close) {
        ...
        time_t tt = time(NULL);
        PUBNUB_LOG_DEBUG(
            "should_keep_alive(pb=%p): pb->keep_alive.count = %d, "
            "pb->keep_alive.max = %d, "
...
static void close_connection(struct pubnub_* pb) {
    if (pbpal_close(pb) <= 0) {
        ...
        PUBNUB_LOG_TRACE(
            "close_connection(): pb->flags.retry_after_close=%d\n",
...
static enum pubnub_res parse_pubnub_result(struct pubnub_* pb) {
    ...
    if (pbres != PNR_OK) {
        PUBNUB_LOG_WARNING(
            "pb=%p parsing response for transaction type #%d "
            "returned error %d\nResponse was: %s\n",
            pb,
            pb->trans,
            pbres,
    ...
    switch (pb->state) {
        ...
        case PBS_CONNECTED:
            ...
            if ((NULL == pb->pal.ssl) && pb->flags.trySSL
                ...
                switch (res) {
                    ...
                    default:
                        PUBNUB_LOG_ERROR(
                            "Unexpected result: pbpal_start_tls()=%d\n",
                            res);
                        outcome_detected(pb, PNR_INTERNAL_ERROR);
                        return 0;
                }
        ...
        case PBS_WAIT_TLS_CONNECT: {
            ...
            switch (res) {
                ...
                default:
                    PUBNUB_LOG_ERROR(
                        "Unexpected result: pbpal_check_tls()=%d\n",
                        res);
                    outcome_detected(pb, PNR_INTERNAL_ERROR);
                    break;
            }
            ...
            if (i <= 0) {
                ...
                switch (pb->proxy_type) {
                    ...
                    case pbproxyNONE:
                        ...
                        if ((i < 0)
                            || (-1 == pbpal_send_str(pb, pb->core.http_buf))) {
                            outcome_detected(pb, PNR_IO_ERROR);
                        }
                        break;
                    default:
        ...
        case PBS_RX_HTTP_VER:
            ...
            switch (pbrslt) {
                ...
                default:
                    PUBNUB_LOG_ERROR(
                        "pb=%p in PBS_RX_HTTP_VER: failure inducing "
        ...
        case PBS_RX_HEADER_LINE:
            pbrslt = pbpal_line_read_status(pb);
            PUBNUB_LOG_TRACE(
                "pb=%p PBS_RX_HEADER_LINE: pbrslt=%d\n",
                pb,
                pbrslt);
            switch (pbrslt) {
                case PNR_IN_PROGRESS:
            ...
            */
                    ...
                    if (pb_strncasecmp(
                        ...
                        == 0) {
                        size_t len =
                            atoi(pb->core.http_buf + sizeof h_length - 1);
        ...
        case PBS_RX_BODY:
            if (pb->core.http_buf_len < pb->core.http_content_len) {
                pbpal_start_read(
                    pb,
                    pb->core.http_content_len - pb->core.http_buf_len);
        ...
        case PBS_RX_CHUNK_LEN_LINE:
            pbrslt = pbpal_line_read_status(pb);
            PUBNUB_LOG_TRACE(
                "pb=%p PBS_RX_CHUNK_LEN_LINE: pbrslt=%d\n",
                pb,
                pbrslt);
            switch (pbrslt) {
                case PNR_IN_PROGRESS:
                ...
                case PNR_OK: {
                    unsigned chunk_length =
                        strtoul(pb->core.http_buf, NULL, 16);

                    PUBNUB_LOG_TRACE(
                        "About to read a chunk w/length: %u\n",
                        chunk_length);
                    if (chunk_length == 0) {
                        finish(pb);
#if PUBNUB_PROXY_API
                        ...
                        != pbcc_realloc_reply_buffer(
                            &pb->core,
                            pb->core.http_buf_len + chunk_length)) {
                        outcome_detected(pb, PNR_REPLY_TOO_BIG);
                    } else {
                        pb->core.http_content_len =
                            chunk_length + CHUNK_TRAIL_LENGTH;
        ...
        case PBS_RX_BODY_CHUNK_WAIT:
            pbrslt = pbpal_read_status(pb);
            PUBNUB_LOG_TRACE(
                "pb=%p PBS_RX_BODY_CHUNK_WAIT: pbrslt=%d\n",
                pb,
                pbrslt);
            switch (pbrslt) {
                case PNR_IN_PROGRESS:
        ...
        default:
            PUBNUB_LOG_ERROR(
                "pbnc_fsm(pb=%p): unhandled state: %s\n",
                pb,
                pbnc_state2str(pb->state));
            break;
    }
...
void pbnc_stop(struct pubnub_* pbp, enum pubnub_res outcome_to_report) {
    ...
    switch (pbp->state) {
        ...
        case PBS_NULL:
            PUBNUB_LOG_ERROR(
                "pbnc_stop(pbp=%p) got called in NULL state\n",
                pbp);
            break;
        case PBS_IDLE:
            pbp->trans = PBTT_NONE;
