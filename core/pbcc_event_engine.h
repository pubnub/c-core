 ...
 *        definition.
 *
 * @param data Pointer to the user-provided data, which should free up used
 *             resources.
 */
...
/**
 * @brief Retrieve current Event Engine state.
 *
 * @param ee Pointer to the Event Engine for which current state should be
 *           returned.
 * @return The current state in which the Event Engine is at this moment.
 */
 ...
 * @param event Event Engine state changing which should be processed.
 ...
 *
 * @param ee Pointer to the Event Engine which should dequeue and process
 *           next invocation.
 ...
 * @return Pointer to the read to use Event Engine data or `NULL` in case of
 *         insufficient memory error. The returned pointer must be passed to
 ...
 *             specific Event Engine event by event emitting code or `NULL` in
 *             case of insufficient memory error. The returned pointer must be
 *             passed to the `pbcc_ee_state_free` to avoid a memory leak.
 * @return Pointer to the Event Engine state.
 */
pbcc_ee_state_t* pbcc_ee_state_alloc(int type, pbcc_ee_data_t* data);
 ...
 *                   Event Engine will enter `state` as current state.
