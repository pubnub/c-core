...
MAYBE_INLINE void pbntf_trans_outcome_sync(
    pubnub_t*         pb,
...
MAYBE_INLINE enum pubnub_res pubnub_last_result_sync(pubnub_t* pb)
{
    ...
    if (!pbnc_can_start_transaction(pb)) { pbnc_fsm((pubnub_t*)pb); }
    result = pb->core.last_result;
    if (result != PNR_OK) { pbnc_tr_cxt_state_reset_sync(pb); }
...
#ifdef PUBNUB_NTF_RUNTIME_SELECTION
    if (PNA_SYNC != pb->api_policy) { return PNR_INTERNAL_ERROR; }
