...
static void fill_sspi_identity(
...
void pbntlm_packer_init(pbntlm_ctx_t* pb) {
    ...
    if (sspi_max_token <= PUBNUB_NTLM_MAX_TOKEN) {
        PUBNUB_LOG_WARNING(
            "PUBNUB_NTLM_MAX_TOKEN(==%d) is smaller than maximum NTLM token for SSPI, which is: %lu\n",
...
static void fill_sec_buf_dec_out(
    SecBuffer* sec_buf,
    SecBufferDesc* sec_desc,
    pubnub_bymebl_t const* msg) {
...
acquire_creds(PSEC_WINNT_AUTH_IDENTITY pident, CredHandle* phcreds) {
    TimeStamp expiry;
...
static SECURITY_STATUS init_sec_ctx(
    CredHandle* phcreds,
    CtxtHandle* phctx,
    SecBufferDesc* in_desc,
    CtxtHandle* phctx_new,
...
int pbntlm_pack_type_one(
    ...
    if (status != SEC_E_OK) {
        PUBNUB_LOG_ERROR(
            "Failed to acquire credentials for NTLM, code: %ld\n",
            status);
        return -1;
    }
    ...
    switch (status) {
        ...
        default:
            PUBNUB_LOG_ERROR(
                "Failed to init context for NTLM type 1 message, code: %ld\n",
                status);
            return -1;
    }
}
...
int pbntlm_unpack_type2(pbntlm_ctx_t* pb, pubnub_bymebl_t msg) {
    ...
    if (msg.size > sizeof pb->in_token) {
        PUBNUB_LOG_ERROR(
            "NTLM Type2 message too long: %zu bytes, max allowed %zu\n",
            msg.size,
            sizeof pb->in_token);
        return -1;
    }
