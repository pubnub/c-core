...
static SECURITY_STATUS acquire_creds(
    PSEC_WINNT_AUTH_IDENTITY pident,
    CredHandle*              phcreds)
{
    TimeStamp expiry;
    ...
    pubnub_bymebl_t* msg)
{
    ...
    if (status != SEC_E_OK) {
        PUBNUB_LOG_ERROR(
            context,
            "NTLM credentials acquire failed with error code: %ld",
            status);
        return -1;
    }
    ...
    default:
        PUBNUB_LOG_ERROR(
            context,
            "NTLM type 1 context initialization failed with error code: %ld",
            status);
        return -1;
    }
}
    ...
    pubnub_bymebl_t* msg)
{
    ...
    if (status != SEC_E_OK) {
        PUBNUB_LOG_ERROR(
            context,
            "NTLM type 3 handshake failed with error code: %lx",
            status);
        return -1;
    }
