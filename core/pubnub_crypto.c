...
static int memory_encode(
    ...
    if (*n + 1 < (size_t)max_size) {
        PUBNUB_LOG_DEBUG(
            "base64encode needs %d bytes but only %zu bytes are available\n",
            max_size,
            *n);
        return -1;
    }
    ...
    if (base64encode(base64_output, max_size, buffer.ptr, buffer.size) != 0) {
        PUBNUB_LOG_DEBUG(
            "base64encode tried to use more than %d bytes to encode %zu bytes\n",
            max_size,
            buffer.size);
        free(base64_output);
        return -1;
    ...
    *slen = 0;
    ...
    do {
        ...
        *slen = req;
        ...
        if (req != *slen) {
            printf(
                "EVP_DigestSignFinal failed, mismatched signature sizes %d, %d\n",
                req,
                *slen);
            break; /* failed */
        }
...
int base64encode(
    ...
    size_t size = EVP_EncodeBlock(
        buf,
        (const unsigned char*)b64_encode_this,
        encode_this_many_bytes);
...
char* pn_pam_hmac_sha256_sign(char const* key, char const* message) {
    ...
    if (base64encode(base64_encoded, max_size, result, bytes_to_encode) != 0) {
        PUBNUB_LOG_DEBUG(
            "base64encode tried to use more than %d bytes to encode %d bytes\n",
            max_size,
            bytes_to_encode);
        free(base64_encoded);
        return NULL;
...
enum pubnub_res pn_gen_pam_v2_sign(
    ...
    int str_to_sign_len = strlen(p->core.subscribe_key)
        ...
        + strlen(qs_to_sign);
    char* str_to_sign = (char*)malloc(
        sizeof(char) * str_to_sign_len + 5);  // 4 variables concat + 1
    if (str_to_sign != NULL) {
        sprintf(
            str_to_sign,
            "%s\n%s\n%s\n%s",
...
enum pubnub_res pn_gen_pam_v3_sign(
    ...
    int str_to_sign_len = strlen(method_verb) + strlen(p->core.publish_key)
        + strlen(partial_url) + strlen(qs_to_sign) + 4 * strlen("\n")
