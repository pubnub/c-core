...
int pbcrypto_signature(
    struct pbcc_context* pbcc,
    char const*          channel,
    char const*          msg,
    char*                signature,
    size_t               n)
{
    ...
    if (NULL == pbcc->secret_key) { return -1; }
    ...
    size_t*         n)
{
    ...
    if (NULL == encrypted.ptr) { return -1; }
    ...
    pubnub_bymebl_t buffer)
{
    ...
    if (-1 == pbaes256_encrypt(msg, key, iv, &buffer)) { return -1; }
...
pubnub_bymebl_t pubnub_decrypt_alloc(
    char const* cipher_key,
    char const* base64_str)
{
    pubnub_bymebl_t decoded;
    ...
    size_t*     n)
{
    ...
    PUBNUB_LOG_DEBUG(pb, "Decrypting next message...");

    msg = (char*)pubnub_get(pb);
    if (NULL == msg) {
        PUBNUB_LOG_ERROR(pb, "There is no more messages in the buffer.");
    ...
    if (NULL != pb->core.crypto_module) {
        PUBNUB_LOG_WARNING(
            pb,
            "PubNub context configured with crypto module configured. Skipping "
            "decryption.");
        s  = msg;
        *n = msg_len;
        return PNR_OK;
    ...
    if ((msg[0] != '"') || (msg[msg_len - 1] != '"')) {
        PUBNUB_LOG_ERROR(pb, "Message format error: not enclosed in quotes.");
    ...
    if (0 != pubnub_decrypt_buffered(cipher_key, msg, &data, &buffer)) {
        PUBNUB_LOG_ERROR(pb, "Decryption failed.");
        return PNR_INTERNAL_ERROR;
    }
    *n                  = data.size;
...
pubnub_bymebl_t pubnub_get_decrypted_alloc(pubnub_t* pb, char const* cipher_key)
{
    ...
    PUBNUB_LOG_DEBUG(
        pb, "Decrypting next message (with allocated response)...");

    msg = (char*)pubnub_get(pb);
    if (NULL == msg) { return result; }
    msg_len = strlen(msg);
    ...
    if ((msg[0] != '"') || (msg[msg_len - 1] != '"')) { return result; }
    msg[msg_len - 1] = '\0';
    ++msg;
    ...
    if (NULL != result.ptr) { result.ptr[result.size] = '\0'; }
...
enum pubnub_res pubnub_publish_encrypted(
    ...
    char const* cipher_key)
{
    PUBNUB_LOG_DEBUG(p, "Publish encrypted message to channel: %s", channel);
...
#if PUBNUB_CRYPTO_API
    PUBNUB_LOG_DEBUG(p, "Set secret key: %s", secret_key);
    ...
    size_t*              slen)
{
    ...
    EVP_PKEY* pkey = EVP_PKEY_new_mac_key(
        EVP_PKEY_HMAC, NULL, (const unsigned char*)key, strlen(key));
    ...
    *slen = 0;
    ...
    do {
        ...
        if (md == NULL) {
            printf(
                "EVP_get_digestbyname failed, error 0x%x\n", ERR_get_error());
            break; /* failed */
        }
        ...
        if (rc != 1) {
            printf(
                "EVP_DigestSignUpdate failed, error 0x%x\n", ERR_get_error());
            break; /* failed */
        }
        ...
        if (rc != 1) {
            printf(
                "EVP_DigestSignFinal failed (1), error 0x%x\n",
                ERR_get_error());
            break; /* failed */
        }
        ...
        if (!(req > 0)) {
            printf(
                "EVP_DigestSignFinal failed (2), error 0x%x\n",
                ERR_get_error());
            break; /* failed */
        }
        ...
        *slen = req;
        ...
        if (rc != 1) {
            printf(
                "EVP_DigestSignFinal failed (3), return code %d, error 0x%x\n",
                rc,
                ERR_get_error());
            break; /* failed */
        }
        ...
        if (req != *slen) {
            printf(
                "EVP_DigestSignFinal failed, mismatched signature sizes %d, "
                "%d\n",
                req,
                *slen);
            break; /* failed */
...
{
    size_t buf_size =
        4 * ((encode_this_many_bytes + 2) / 3) + 1; // +1 for tailing 0
    unsigned char* buf = (unsigned char*)calloc(buf_size, 1);
    if (!buf) { return -1; }

    size_t size = EVP_EncodeBlock(
        buf, (const unsigned char*)b64_encode_this, encode_this_many_bytes);
...
char* pn_pam_hmac_sha256_sign(char const* key, char const* message)
{
    ...
    result                   = mx_hmac_sha256(
        (const void*)key, keylen, msgdata, datalen, result, &resultlen);
#endif
    int bytes_to_encode = (int)resultlen;
    if (bytes_to_encode <= 0) { return NULL; }
    ...
    while (base64_encoded[counter] != '\0') {
        if (base64_encoded[counter] == '+') { base64_encoded[counter] = '-'; }
    ...
    size_t      signature_len)
{
    ...
    int             str_to_sign_len = strlen(p->core.subscribe_key) +
                          strlen(p->core.publish_key) + strlen(partial_url) +
                          strlen(qs_to_sign);
    size_t str_to_sign_size = sizeof(char) * str_to_sign_len + 5;
    char*  str_to_sign =
        (char*)malloc(str_to_sign_size); // 4 variables concat + 1
    if (str_to_sign != NULL) {
        snprintf(
            str_to_sign,
            str_to_sign_size,
            "%s\n%s\n%s\n%s",
            p->core.subscribe_key,
            p->core.publish_key,
            partial_url,
            qs_to_sign);
    }
    ...
    size_t      signature_len)
{
    ...
    int str_to_sign_len = strlen(method_verb) + strlen(p->core.publish_key) +
                          strlen(partial_url) + strlen(qs_to_sign) +
...
static pubnub_bymebl_t provider_encrypt(
    struct pubnub_crypto_provider_t const* provider,
    pubnub_bymebl_t                        to_encrypt);
static pubnub_bymebl_t provider_decrypt(
    struct pubnub_crypto_provider_t const* provider,
    pubnub_bymebl_t                        to_decrypt);


struct pubnub_crypto_provider_t* pubnub_crypto_module_init(
    struct pubnub_cryptor_t* default_algorithm,
    struct pubnub_cryptor_t* algorithms,
    size_t                   algorithms_count)
{
    struct pubnub_crypto_provider_t* crypto =
        (struct pubnub_crypto_provider_t*)malloc(
            sizeof(struct pubnub_crypto_provider_t));
    if (crypto == NULL) { return NULL; }

    struct crypto_module* module =
        (struct crypto_module*)malloc(sizeof(struct crypto_module));
    if (module == NULL) {
        free(crypto);
        return NULL;
    }
...
struct pubnub_crypto_provider_t* pubnub_crypto_aes_cbc_module_init(
    const uint8_t* cipher_key)
{
    return pubnub_crypto_module_init(
        pbcc_aes_cbc_init(cipher_key), pbcc_legacy_crypto_init(cipher_key), 1);
}


struct pubnub_crypto_provider_t* pubnub_crypto_legacy_module_init(
    const uint8_t* cipher_key)
{
    return pubnub_crypto_module_init(
        pbcc_legacy_crypto_init(cipher_key), pbcc_aes_cbc_init(cipher_key), 1);
}
    ...
    pubnub_bymebl_t                        to_encrypt)
{
    ...
    struct pubnub_encrypted_data* encrypted_data =
        (struct pubnub_encrypted_data*)malloc(
            sizeof(struct pubnub_encrypted_data));

    if (0 != algorithm->encrypt(algorithm, encrypted_data, to_encrypt)) {
        return result;
    };

    struct pubnub_cryptor_header_v1 header = pbcc_prepare_cryptor_header_v1(
        algorithm->identifier, encrypted_data->metadata);
    ...
    if (result.ptr == NULL) { return result; }

    result.size = whole_size;

    struct pubnub_byte_mem_block* header_block =
        pbcc_cryptor_header_v1_to_alloc_block(&header);
    ...
    if (encrypted_data->metadata.size > 0 &&
        encrypted_data->metadata.ptr != NULL) {
        memcpy(
            result.ptr + header_size - encrypted_data->metadata.size,
            encrypted_data->metadata.ptr,
            encrypted_data->metadata.size);
    }

    memcpy(
        result.ptr + header_size,
        encrypted_data->data.ptr,
...
static pubnub_cryptor_t* cryptor_with_identifier(
    struct crypto_module*            module,
    struct pubnub_cryptor_header_v1* header)
{
    const char* identifier = (char*)header->identifier;
    if (NULL == header) { identifier = PUBNUB_LEGACY_CRYPTO_IDENTIFIER; }

    if (0 == memcmp(
                 (char*)module->default_algorithm->identifier,
                 identifier,
                 PUBNUB_CRYPTOR_HEADER_IDENTIFIER_SIZE)) {
        return module->default_algorithm;
    }

    for (size_t i = 0; i < module->algorithms_count; i++) {
        if (0 == memcmp(
                     (char*)module->algorithms[i].identifier,
                     identifier,
                     PUBNUB_CRYPTOR_HEADER_IDENTIFIER_SIZE)) {
    ...
    pubnub_bymebl_t                        to_decrypt)
{
    ...
    if (NULL == to_decrypt.ptr || to_decrypt.size == 0) { return result; }

    struct crypto_module* module = (struct crypto_module*)provider->user_data;

    struct pubnub_cryptor_header_v1* header =
        pbcc_cryptor_header_v1_from_block(&to_decrypt);

    struct pubnub_cryptor_t* algorithm =
        cryptor_with_identifier(module, header);

    if (NULL == algorithm) { return result; }
...
void pubnub_set_crypto_module(
    pubnub_t*                        pubnub,
    struct pubnub_crypto_provider_t* crypto_provider)
{
    PUBNUB_LOG_DEBUG(
        pubnub,
        crypto_provider ? "Crypto module enabled" : "Crypto module disabled");
