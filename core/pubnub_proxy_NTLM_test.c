...
enum pbpal_resolv_n_connect_result pbpal_resolv_and_connect(pubnub_t* pb) {
    return (enum pbpal_resolv_n_connect_result)
        mock("pbpal_resolv_and_connect", pb, "");
}

enum pbpal_resolv_n_connect_result
pbpal_check_resolv_and_connect(pubnub_t* pb) {
    return (enum pbpal_resolv_n_connect_result)
        mock("pbpal_check_resolv_and_connect", pb, "");
}
...
enum pbpal_resolv_n_connect_result pbpal_check_connect(pubnub_t* pb) {
    return (
        enum pbpal_resolv_n_connect_result)mock("pbpal_check_connect", pb, "");
}
...
static void
expect_have_dns_for_proxy_server_without_enqueue_for_processing(void) {
    expect("pbpal_resolv_and_connect", pbp, "", pbpal_connect_success);
    expect("pbntf_got_socket", pbp, "", 0);
}
...
static void
CONNECT_NTLM_proxy_closes_connection_on_407_dialogue_continues(void) {
    int proxy_port = 500;
    BeforeEach();
    pubnub_init(pbp, "publ-key", "sub-key");
    attest(
