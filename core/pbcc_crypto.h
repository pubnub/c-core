...
typedef struct pubnub_cryptor_t {
    ...
    /** Function pointer to the encrypt function.
        ...
        @return 0: OK, -1: error
      */
    int (*encrypt)(struct pubnub_cryptor_t const* cryptor,
     ...
     */
    int (*decrypt)(struct pubnub_cryptor_t const* cryptor,
...
typedef struct pubnub_crypto_provider_t {
    /** Function pointer to the encrypt function.
      ...
      */
    pubnub_bymebl_t (*encrypt)(struct pubnub_crypto_provider_t const* provider,
                               pubnub_bymebl_t to_encrypt);
     ...
     */
    pubnub_bymebl_t (*decrypt)(struct pubnub_crypto_provider_t const* provider,
...
*/
struct pubnub_cryptor_header_v1
pbcc_prepare_cryptor_header_v1(uint8_t                      identifier[4],
                               struct pubnub_byte_mem_block metadata);
...
*/
struct pubnub_cryptor_header_v1*
pbcc_cryptor_header_v1_from_block(struct pubnub_byte_mem_block* cryptor_header);
...
/**
    ...
    @return decrypted data block
*/
int pbcc_legacy_decrypt(uint8_t const*   cipher_key,
                        pubnub_bymebl_t* result,
                        pubnub_bymebl_t  to_decrypt);
...
/**
    ...
    This function encodes the encrypted data to base64. It allocates the memory
   for the encoded data and returns it to the user.

    @param pb Pointer to the PubNub context which is used to base64-encode data.
...
/**
    ...
    This function decodes the encrypted data from base64. It allocates the
   memory for the decoded data and returns it to the user.
...
*/
void pbcc_set_crypto_module(struct pbcc_context*             ctx,
...
*/
const char* pbcc_decrypt_message(struct pbcc_context* ctx,
                                 const char*          message,
                                 size_t               len,
                                 size_t*              out_len);
