 ...
 * APIs. If the client is built with only one API, the policy is not even
 * compiled.
 */
 ...
 * @see pubnub_res
 */
PUBNUB_EXTERN void pubnub_enforce_api(
    pubnub_t*                   pb,
    enum pubnub_api_enforcement policy);
...
int pbntf_init_callback(pubnub_t* pb);
