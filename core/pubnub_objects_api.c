...
{
    struct pubnub_set_uuidmetadata_opts opts = pubnub_set_uuidmetadata_defopts();
    opts.uuid = uuid_metadataid;
    opts.include = include;
    opts.data.custom = uuid_metadata_obj;

    return pubnub_set_uuidmetadata_ex(pb, opts);
}
    ...
    struct pubnub_set_uuidmetadata_opts opts)
{
    ...
    rslt = pbcc_set_uuidmetadata_prep(&pb->core, opts.uuid, opts.include, opts.data.custom /*remove .custom*/, pb->trans);
    if (PNR_STARTED == rslt) {
        pb->trans            = PBTT_SET_UUIDMETADATA;
        pb->core.last_result = PNR_STARTED;
        pb->method           = pubnubUsePATCH;
...
{
    struct pubnub_set_channelmetadata_opts opts = pubnub_set_channelmetadata_defopts();
    opts.include = include;
    opts.data.custom = channel_metadata_obj;

    return pubnub_set_channelmetadata_ex(pb, channel_metadataid, opts);
}
        ...
        struct pubnub_set_channelmetadata_opts opts)
{
    ...
    rslt = pbcc_set_channelmetadata_prep(&pb->core,
                                  channel,
                                  opts.include,
                                  opts.data.custom /*remove .custom*/,
                                  pb->trans);
    if (PNR_STARTED == rslt) {
        pb->trans            = PBTT_SET_CHANNELMETADATA;
        pb->core.last_result = PNR_STARTED;
...
struct pubnub_members_data pubnub_get_members_defopts(void)
{
...
enum pubnub_res pubnub_get_members(pubnub_t* pb,
                                   ...
                                   enum pubnub_tribool count)
{
    struct pubnub_members_data opts = pubnub_get_members_defopts();
    opts.include = include;
    opts.limit = limit;
    opts.page.next = start;
    opts.page.prev = end;
