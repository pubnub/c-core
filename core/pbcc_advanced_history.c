...
enum pubnub_res pbcc_parse_message_counts_response(struct pbcc_context* p) {
    ...
    if ((reply[0] != '{') || (reply[replylen - 1] != '}')) {
        PUBNUB_LOG_ERROR(
            "Error: pbcc_parse_message_counts_response(pbcc=%p) - "
...
int pbcc_get_error_message(struct pbcc_context* p, pubnub_chamebl_t* o_msg) {
    ...
    if (jonmpOK == jpresult) {
        ...
        PUBNUB_LOG_ERROR(
            "Error: pubnub_get_error_message(pbcc=%p) - "
...
int pbcc_get_chan_msg_counts(
    ...
    while ((ch_start < end) && (count < *io_count)) {
        ...
        if (*ch_start != ':') {
            PUBNUB_LOG_DEBUG(
                "Note: pubnub_get_chan_msg_counts(pbcc=%p) - "
...
int pbcc_get_message_counts(
    ...
    while ((ch_start < end) && (counts < n)) {
        ...
        if (i == n) {
            PUBNUB_LOG_DEBUG(
                "Note: pubnub_get_msg_counts(pbcc=%p) - "
...
static enum pubnub_parameter_error
check_timetoken(struct pbcc_context* p, char const* timetoken) {
    size_t len = strlen(timetoken);
    if (len < PUBNUB_MIN_TIMETOKEN_LEN) {
        PUBNUB_LOG_ERROR(
            "Error: message_counts_prep(pbcc=%p) - "
...
static enum pubnub_parameter_error
check_channels(struct pbcc_context* p, char const* channels) {
    size_t len;
...
static enum pubnub_parameter_error check_channel_timetokens(
    ...
    while (next != NULL) {
        if (NULL == next_within_timetokens) {
            PUBNUB_LOG_ERROR(
                "Error: message_counts_prep(pbcc=%p) - Number of channels and "
    ...
    if (strspn(channel_timetokens, OK_SPAN_CHARACTERS)
        ...
        PUBNUB_LOG_ERROR(
            "Error: message_counts_prep(pbcc=%p) - Invalid channel_timetokens."
            "(channel_timetokens='%s')\n",
            p,
            channel_timetokens);
        return pnarg_INVALID_TIMETOKEN;
...
static enum pubnub_parameter_error check_parameters(
    ...
    if (channel_timetokens != NULL) {
        enum pubnub_parameter_error rslt =
            check_channel_timetokens(p, channel, channel_timetokens);
...
#endif
    if (timetoken) {
        ADD_URL_PARAM(qparam, timetoken, timetoken);
    }
