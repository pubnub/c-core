...
struct pubnub_cryptor_t* pbcc_aes_cbc_init(const uint8_t* cipher_key)
{
    ...
    struct aes_context* ctx =
        (struct aes_context*)malloc(sizeof(struct aes_context));
    if (ctx == NULL) {
        free(algo);
        return NULL;
    }
    ...
    pubnub_bymebl_t                to_encrypt)
{
    ...
    if (result->data.ptr == NULL) { return -1; }
    ...
    struct pubnub_encrypted_data   to_decrypt)
{
    ...
    if (result->ptr == NULL) { return -1; }

    result->size = dec_buffer_size;

    if (0 != pbaes256_decrypt(
                 to_decrypt.data, key_hash, to_decrypt.metadata.ptr, result)) {
        free(result->ptr);
        return -1;
    }
