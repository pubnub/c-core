...
int pubnub_get_grant_bit_mask_value(struct pam_permission perm) {
    ...
    if (perm.write) {
        result = result | PERM_WRITE;
    }
    if (perm.manage) {
        result = result | PERM_MANAGE;
    }
    if (perm.del) {
        result = result | PERM_DELETE;
    }
    if (perm.create) {
        result = result | PERM_CREATE;
    }
    if (perm.get) {
        result = result | PERM_GET;
    }
    if (perm.update) {
        result = result | PERM_UPDATE;
    }
    if (perm.join) {
        result = result | PERM_JOIN;
    }
    return result;
}
...
pubnub_chamebl_t pubnub_get_grant_token(pubnub_t* pb) {
    ...
    if (pb->trans != PBTT_GRANT_TOKEN) {
        PUBNUB_LOG_ERROR(
            "pubnub_get_grant_token(pb=%p) can be called only if "
                ...
                if (cbor_value_get_type(it) != CborInvalidType) {
                    ...
                    if (sig_flag) {
                        ...
                        if (encoded_sig.size == 0 && encoded_sig.ptr == NULL) {
                            PUBNUB_LOG_WARNING(
                                "\"sig\" field coudn't be encoded! Leaving it empty!");
                        ...
                        char base64_str
                            [67];  // HMAC+SHA256 max size + quotes + tailing null
...
char* pubnub_parse_token(pubnub_t* pb, char const* token) {
    ...
    if (decoded.size == 0 && decoded.ptr == NULL) {
        PUBNUB_LOG_ERROR(
            "Base64 decoding failed! Token \"%s\" is not a valid base64 value!\n",
            token);
