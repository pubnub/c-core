...
*/
    ...
    #define PUBNUB_LOG_ERROR(...) \
        PUBNUB_LOG(PUBNUB_LOG_LEVEL_ERROR, __VA_ARGS__)

    /** Helper macro to log a warning message */
    #define PUBNUB_LOG_WARNING(...) \
        PUBNUB_LOG(PUBNUB_LOG_LEVEL_WARNING, __VA_ARGS__)
    ...
    #define PUBNUB_LOG_DEBUG(...) \
        PUBNUB_LOG(PUBNUB_LOG_LEVEL_DEBUG, __VA_ARGS__)

    /** Helper macro to log a tracing message */
    #define PUBNUB_LOG_TRACE(...) \
        PUBNUB_LOG(PUBNUB_LOG_LEVEL_TRACE, __VA_ARGS__)

    /** Generic macro to print a value of a (scalar) variable.
 */
    #define WATCH_VAL(X, T, FMT) \
        do { \
            (void)(1 ? &X : (T*)0); \
            PUBNUB_LOG( \
                PUBNUB_LOG_LEVEL_DEBUG, \
                __FILE__ "(%d) in %s: `" #X "` = " FMT "\n", \
                __LINE__, \
                __FUNCTION__, \
                X); \
        } while (0)
 ...
 */
    #define WATCH_SIZE_T(X) \
        do { \
            unsigned long M_watched_val_ = (X); \
            WATCH_ULONG(M_watched_val_); \
        } while (0)
    ...
    #define WATCH_BYTEARR(X) \
        do { \
            unsigned char const* ba_ = (X); \
            PUBNUB_LOG( \
                PUBNUB_LOG_LEVEL_DEBUG, \
                __FILE__ "(%d) in %s: `" #X "` = '%s'\n", \
                __LINE__, \
