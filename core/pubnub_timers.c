...
int pubnub_set_transaction_timeout(pubnub_t* p, int duration_ms)
{
    ...
    if (duration_ms < PUBNUB_MIN_TRANSACTION_TIMER) {
        PUBNUB_LOG_WARNING(
            p,
            "Using minimal transaction timeout instead: %d ms",
    ...
    PUBNUB_LOG_DEBUG(p, "Transaction timeout is set to: %d ms", duration_ms);
    return 0;
}
...
int pubnub_set_wait_connect_timeout(pubnub_t* p, int duration_ms)
{
    ...
    if (duration_ms < PUBNUB_MIN_WAIT_CONNECT_TIMER) {
        PUBNUB_LOG_WARNING(
            p,
            "Using minimal connection wait timeout instead: %d ms",
    ...
    pubnub_mutex_unlock(p->monitor);
    PUBNUB_LOG_DEBUG(p, "Wait connect timeout is set to: %d ms", duration_ms);
    return 0;
}
