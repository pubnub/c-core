...
enum pubnub_res pbcc_subscribe_v2_prep(
    ...
    if (tt != NULL) {
        ADD_URL_PARAM(qparam, tt, tt);
    }
    if (tr) {
        ADD_URL_PARAM(qparam, tr, tr);
    }
    ...
    if (filter_expr) {
        ADD_URL_PARAM(qparam, filter - expr, filter_expr);
    }
    if (heartbeat) {
...
enum pubnub_res pbcc_parse_subscribe_v2_response(struct pbcc_context* p) {
    ...
    if (pbjson_value_for_field_found(&el, "status", "403")) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_subscribe_v2_response(pbcc=%p) - AccessDenied: "
...
struct pubnub_v2_message pbcc_get_msg_v2(struct pbcc_context* p) {
    ...
    */
    ...
    if (jonmpOK == jpresult) {
        ...
        PUBNUB_LOG_ERROR(
            "pbcc=%p: No message payload in subscribe V2 response "
            "found, error=%d\n",
            p,
            jpresult);
        return rslt;
    ...
    if (jonmpOK == jpresult) {
        ...
        PUBNUB_LOG_ERROR(
            "pbcc=%p: No message channel in subscribe V2 response "
            "found, error=%d\n",
            p,
            jpresult);
        return rslt;
    ...
    if (jonmpOK == jpresult) {
        ...
        PUBNUB_LOG_ERROR(
            "No message publish timetoken in subscribe V2 "
