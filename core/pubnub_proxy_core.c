...
int pbproxy_check_realm(pubnub_chamebl_t const* value) {
    if (value->size > PUBNUB_MAX_HTTP_AUTH_REALM + 1) {
        PUBNUB_LOG_ERROR(
            "Received `realm` too long: %zu, maximum possible %d\n",
            value->size,
    ...
    if (value->size <= 2) {
        PUBNUB_LOG_ERROR(
            "Received 'realm' is too short (length = %zu)\n",
            value->size);
        return -1;
    }
    return 0;
...
int pbproxy_handle_http_header(pubnub_t* p, char const* header) {
    ...
    PUBNUB_LOG_TRACE(
        "pbproxy_handle_http_header(header='%s', contents='%s')\n",
        header,
        contents);
    ...
    if ((0 == strncmp(contents, scheme_basic, sizeof scheme_basic - 1))
        ...
        do {
            if (NULL == s) {
                PUBNUB_LOG_WARNING(
                    "Warning: haven't got realm for basic authentication\n");
        ...
        PUBNUB_LOG_WARNING(
            "Warning: Proxy Authentication '%.*s' not supported\n",
...
int pbproxy_http_header_to_send(pubnub_t* p, char* header, size_t n) {
    ...
    switch (p->proxy_auth_scheme) {
        case pbhtauBasic: {
            ...
            PUBNUB_LOG_TRACE(
                "pbproxy_http_header_to_send(): Basic header (before "
                "Base64): '%s'\n",
                response);
            ...
            if (0 == i) {
                PUBNUB_LOG_TRACE(
                    "pbproxy_http_header_to_send(): Basic header "
                    "(after Base64): '%s'\n",
                    header);
                p->proxy_authorization_sent = true;
            } else {
                PUBNUB_LOG_ERROR(
                    "pbproxy_http_header_to_send(): Basic Failed "
        ...
        case pbhtauDigest: {
            ...
            pubnub_chamebl_t data = {
                header + sizeof prefix - 1,
                n - (sizeof prefix - 1)};

            memcpy(header, prefix, sizeof prefix);

            if (0
                == pbhttp_digest_prep_header_to_send(
                    &p->digest_context,
                    figure_out_username(p),
                    figure_out_password(p),
                    p->proxy_saved_path,
        ...
        case pbhtauNTLM: {
            ...
            if (i != 0) {
                PUBNUB_LOG_ERROR(
                    "pbproxy_http_header_to_send(): NTLM failed "
            ...
            if (0 == i) {
                PUBNUB_LOG_TRACE(
                    "pbproxy_http_header_to_send(): NTLM header "
                    "(after Base64): '%s'\n",
                    header);
            } else {
                PUBNUB_LOG_ERROR(
        ...
        default:
            PUBNUB_LOG_ERROR(
                "pbproxy_http_header_to_send(): Proxy auth scheme %d "
                "not supported\n",
                p->proxy_auth_scheme);
            return -1;
    }
