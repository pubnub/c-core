...
static enum pubnub_res deflate_total_to_context_buffer(
    pubnub_t*   pb,
    char const* message,
    size_t      message_size)
{
    ...
    case TDEFL_STATUS_DONE:
        if (message_size == unpacked_size) {
            uint32_t crc;
            size_t   packed_size = GZIP_HEADER_LENGTH_BYTES + compressed +
                                 GZIP_FOOTER_LENGTH_BYTES;
            size_t diff = unpacked_size - packed_size;

            PUBNUB_LOG_TRACE(
                pb,
                "%lu bytes compressed to %s bytes with %ld compression rate",
                (unsigned long)unpacked_size,
                (unsigned long)packed_size,
                ((long)(diff * 1000) / (long)unpacked_size));
            if ((packed_size > unpacked_size) ||
                ((diff * 100) / unpacked_size <
            ...
            /* Unpacked message size is placed at the end of the 'gzip' formated
               message in the last four bytes(little endian)
             */
            gzip_msg_buf[packed_size - 1] = (uint32_t)unpacked_size >> 24;
            gzip_msg_buf[packed_size - 2] =
                ((uint32_t)unpacked_size >> 16) & 0xFF;
            gzip_msg_buf[packed_size - 3] =
                ((uint32_t)unpacked_size >> 8) & 0xFF;
            ...
            PUBNUB_LOG_ERROR(
                pb,
                "Incomplete %zu bytes compression with only %zu bytes "
                "compressed.",
                unpacked_size,
                message_size);
        }
    ...
    case TDEFL_STATUS_BAD_PARAM:
        PUBNUB_LOG_ERROR(pb, "Compression failed: bad parameters");
        break;
    case TDEFL_STATUS_OKAY:
    case TDEFL_STATUS_PUT_BUF_FAILED:
        PUBNUB_LOG_ERROR(pb, "Compression failed: buffer too small");
        break;
    default:
        if (status < 0) {
            PUBNUB_LOG_ERROR(
                pb, "Compression failed with error code: %d", status);
        }
        else {
            PUBNUB_LOG_ERROR(pb, "Compression status: %d", status);
        }
        break;
    }
...
PUBNUB_STATIC_ASSERT(
    sizeof(*(pubnub_t*)(NULL)).core.gzip_msg_buf >
        (GZIP_HEADER_LENGTH_BYTES + GZIP_FOOTER_LENGTH_BYTES),
...
enum pubnub_res pbgzip_compress(pubnub_t* pb, char const* message)
{
    ...
    size = strlen(message);
    PUBNUB_LOG_TRACE(
        pb, "Length before compression: %lu bytes", (long unsigned int)size);
