...
struct HeartbeatWatcherData {
    ...
    /** Array of thumper indices for which auto heartbeat transactions are currently in progress.
      ...
      */
    unsigned heartbeat_in_progress_index_array
        [PUBNUB_MAX_HEARTBEAT_THUMPERS] pubnub_guarded_by(mutw);
    ...
    size_t heartbeat_timers[PUBNUB_MAX_HEARTBEAT_THUMPERS] pubnub_guarded_by(
        timerlock);
    /** Array of thumper indexes whos auto heartbeat timers are active and running */
    unsigned timer_index_array[PUBNUB_MAX_HEARTBEAT_THUMPERS] pubnub_guarded_by(
        timerlock);
...
void pbauto_heartbeat_stop(void);
        ...
        #define pbauto_heartbeat_prepare_channels_and_ch_groups( \
            pb, \
            addr_channel, \
            addr_channel_group) \
            PNR_OK
