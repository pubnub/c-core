...
int main() {
    ...
    */
    ...
    sprintf(
        perm_obj,
        "{\"ttl\":%d, \"uuid\":\"%s\", \"permissions\":{\"resources\":{\"channels\":{ \"mych\":31, \"%s\":%d }, \"groups\":{ \"mycg\":31, \"channel-group\":%d }, \"users\":{ \"myuser\":31 }, \"spaces\":{ \"myspc\":31 }}, \"patterns\":{\"channels\":{ }, \"groups\":{ }, \"users\":{ \"^$\":1 }, \"spaces\":{ \"^$\":1 }},\"meta\":{ }}}",
        ttl_minutes,
        authorized_user_id,
        chan,
        perm_hello_world,
    ...
    if (PNR_STARTED == res) {
        ...
        if (PNR_OK == res) {
            ...
            printf(
                "pubnub_grant_token() Response from Pubnub: %s\n",
                grant_token);
            ...
            printf(
                "pubnub_grant_token() failed with code: %d('%s')\n",
                res,
                pubnub_res_2_string(res));
        }
    } else {
    ...
    */
    ...
    if (PNR_OK == res) {
        printf(
            "Access token permissions revoked! Response from Pubnub: %s\n",
            pubnub_get_revoke_token_response(gtp).ptr);
    } else if (PNR_PUBLISH_FAILED == res) {
        printf(
            "pubnub_revoke_token() failed with code: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
    } else {
        printf(
    ...
    if (PNR_OK == res) {
        printf(
            "Unexpected publish success (request should fail)! Response from Pubnub: %s\n",
            pubnub_last_publish_result(pbp));
    } else if (PNR_PUBLISH_FAILED == res) {
        printf(
            "Expected publish failure on Pubnub as expected, description: %s\n",
            pubnub_last_publish_result(pbp));
    } else {
        printf(
            "Expected publish failure with code: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
    }
