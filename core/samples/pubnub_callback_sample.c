...
void sample_callback(
    ...
    switch (trans) {
        case PBTT_PUBLISH:
            printf(
                "Published, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_SUBSCRIBE:
            printf(
                "Subscribed, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_LEAVE:
        ...
        case PBTT_HISTORY:
            printf(
                "Historied, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_HERENOW:
            printf(
                "Here now'ed, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_GLOBAL_HERENOW:
            printf(
                "Global Here now'ed, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_WHERENOW:
            printf(
                "Where now'ed, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_SET_STATE:
            printf(
                "State set, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_STATE_GET:
            printf(
                "State gotten, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_REMOVE_CHANNEL_GROUP:
            printf(
                "Remove channel group, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_REMOVE_CHANNEL_FROM_GROUP:
            printf(
                "Remove channel from group, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_ADD_CHANNEL_TO_GROUP:
            printf(
                "Add channel to group, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        case PBTT_LIST_CHANNEL_GROUP:
            printf(
                "List (members of) channel group, result: %d('%s')\n",
                result,
                pubnub_res_2_string(result));
            break;
        default:
...
static int do_time(pubnub_t* pbp, struct UserData* pUserData) {
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_time() returned unexpected %d('%s')\n",
            res,
...
int main() {
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_history() returned unexpected: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
        callback_sample_free(pbp);
        return -1;
    ...
    if (PNR_OK == res) {
        ...
        printf(
            "Getting historyv2 failed with code: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
    }
    ...
    */
    if (PUBNUB_DYNAMIC_REPLY_BUFFER) {
        ...
        if (res != PNR_STARTED) {
            printf(
                "pubnub_global_here_now() returned unexpected: %d('%s')\n",
                res,
                pubnub_res_2_string(res));
            callback_sample_free(pbp);
            return -1;
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_where_now() returned unexpected: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
        callback_sample_free(pbp);
        return -1;
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_set_state() returned unexpected: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
        callback_sample_free(pbp);
        return -1;
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_state_get() returned unexpected: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
        callback_sample_free(pbp);
        return -1;
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_add_channel_to_group() returned unexpected: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
        callback_sample_free(pbp);
        return -1;
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_remove_channel_from_group() returned unexpected: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
        callback_sample_free(pbp);
        return -1;
    ...
    if (res != PNR_STARTED) {
        printf(
            "pubnub_remove_channel_group() returned unexpected: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
        callback_sample_free(pbp);
        return -1;
