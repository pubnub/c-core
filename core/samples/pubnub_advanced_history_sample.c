...
static char* m_channel[] =
    {"pool_player", "lucky_hand", "wild_card", "fast_draw", "long_shot"};
 ...
 */
static int m_timetoken_index[sizeof m_channel / sizeof m_channel[0]] =
    {2, 1, 4, 3, 2};
...
static void wait_useconds(unsigned long time_in_microseconds) {
    double d_time =
        (double)time_in_microseconds / (1000000.0 / (double)CLOCKS_PER_SEC);
 ...
 */
static void calculate_internal_msg_counts_for_a_single_timetoken(
 ...
 */
static void calculate_internal_msg_counts_for_timetoken_list(
    int* internal_msg_counts,
    int n) {
    int j;
    for (j = 0; j < n; j++) {
...
static void print_message_counts(
    pubnub_t* pbp,
    int index_timetoken,
    int* msg_counts,
    int* internal_msg_counts) {
    ...
    if (index_timetoken > 0) {
        ...
        printf(
            "tt[%d]='%s'|tt[%d]='%s'|tt[%d]='%s'|tt[%d]='%s'|tt[%d]='%s'|\n",
            m_timetoken_index[0],
...
static void await_msg_counts_and_print_result(
    pubnub_t* pbp,
    ...
    printf(
        "Getting message counts lasted %lf seconds.\n",
        difftime(time(NULL), t0));
    if (PNR_OK == res) {
        if (pubnub_get_chan_msg_counts_size(pbp)
            == sizeof m_channel / sizeof m_channel[0]) {
            puts(
                "-----------------------------------Got message counts for all channels!----------------------------------");
        }
        ...
        printf(
            "Getting message counts failed with code: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
    }
}

static void get_message_counts_for_a_single_timetoken(
    pubnub_t* pbp,
    char* string_channels,
    int index_timetoken) {
    time_t t0;
    ...
    calculate_internal_msg_counts_for_a_single_timetoken(
        internal_msg_counts,
        sizeof m_channel / sizeof m_channel[0],
        index_timetoken);
    puts("------------------------------------------------");
...
static void get_message_counts_for_the_list_of_timetokens(
    pubnub_t* pbp,
    char* string_channels) {
    time_t t0;
    ...
    calculate_internal_msg_counts_for_timetoken_list(
        internal_msg_counts,
        sizeof m_channel / sizeof m_channel[0]);
    snprintf(
        string_timetokens,
...
int main(int argc, char* argv[]) {
    ...
    snprintf(
        ...
        m_channel[3],
        m_channel[4]);
    get_message_counts_for_a_single_timetoken(
        pbp_2,
        string_channels,
        m_timetoken_index[0]);
