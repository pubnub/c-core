...
static void
subloop_callback(pubnub_t* pbp, char const* message, enum pubnub_res result) {
    PUBNUB_UNUSED(pbp);
    if (PNR_OK == result) {
        paint_text_blue();
        ...
        paint_text_red();
        printf(
            "Subscribe failed with code:%d('%s')\n",
            result,
    ...
    switch (trans) {
        case PBTT_PUBLISH:
            ...
            if (result == PNR_STARTED) {
                paint_text_red();
                printf(
                    "await() returned unexpected: PNR_STARTED(%d)\n",
                    result);
                m_stop = 1;
                return;
            }
            if (PNR_OK == result) {
                ...
                paint_text_red();
                printf(
                    "Publish failed on Pubnub, description: %s\n",
                    pubnub_last_publish_result(pb));
            } else {
                paint_text_red();
                printf(
            ...
            if (result != PNR_CANCELLED) {
                ...
                if (m_tail != m_head) {
                    ...
                    puts("-----------------------");
                    result =
                        pubnub_publish(pb, (char*)user_data, m_message[m_tail]);
                    if (result == PNR_STARTED) {
                        m_tail = (m_tail + 1)
                            % (sizeof m_message / sizeof m_message[0]);
                    } else {
                        paint_text_red();
                        printf(
                            "pubnub_publish() from callback returned "
                            "unexpected:%d('%s')\n",
                            result,
                            pubnub_res_2_string(result));
                        m_stop = 1;
...
int main() {
    ...
    while (!m_stop) {
        ...
        if (!m_stop && (rand() % 20 < 7)) {
            ++i;
            pubnub_mutex_lock(m_lock);
            if (((m_head + 1) % (sizeof m_message / sizeof m_message[0]))
                != m_tail) {
                paint_text_yellow();
                snprintf(
                    m_message[m_head],
                ...
                m_head =
                    (m_head + 1) % (sizeof m_message / sizeof m_message[0]);
                pubnub_mutex_unlock(m_lock);
            } else {
                pubnub_mutex_unlock(m_lock);
                paint_text_red();
