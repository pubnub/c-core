    ...
    case PBTT_PUBLISH:
        ...
        if (result != PNR_CANCELLED) {
            ...
            if(m_tail != m_head) {
                ...
                if(result == PNR_STARTED) {
                    m_tail = (m_tail + 1)%(sizeof m_message / sizeof m_message[0]);
                }
                else {
                    paint_text_red();
                    printf("pubnub_publish() from callback returned unexpected:%d(%s)\n",
                           result,
    ...
    while (!stop) {
        ...
        if (!stop && (rand() % 20 < 10)) {
            ...
            if(((m_head + 1) % (sizeof m_message / sizeof m_message[0])) != m_tail) {
                paint_text_yellow();
                snprintf(m_message[m_head], sizeof m_message[m_head], "\"%d-hello world from subscribe publish callback queue!\"", i);
