...
int main() {
    ...
    pubnub_bymebl_t no_serialize_data = {
        (uint8_t*)noserialize_msg,
        (size_t)strlen(noserialize_msg)};
    if (0
        != pubnub_encrypt(
            cipher_key,
            no_serialize_data,
    ...
    printf(
        "base64 message = %s (size = %d) \n",
        noserializemsg_encrypted_str,
        (int)noserializemsg_size);
    printf("\n ***************************************************** \n");
    ...
    if (NULL == (&noserializemsg_deencrypted)->ptr) {
        ...
        printf(
            "Decrypt of previous encrypted message = %s (size = %d)\n",
            (&noserializemsg_deencrypted)->ptr,
            (int)(&noserializemsg_deencrypted)->size);
    }
    printf("\n ***************************************************** \n");
        ...
        if (NULL != msg) {
            ...
            if (jonmpOK == pbjson_get_object_value(&el, "message", &found)) {
                ...
                if (msg.ptr != NULL) {
                    if (0 == strcmp((char*)msg.ptr, "\"yay!\"")) {
                        printf(
                            "Decryption gave %s in History. Hurray!\n",
                            msg.ptr);
                    }
                }
            }
