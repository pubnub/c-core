...
publish(pubnub_t* pbp, char const* chan, char const* meta) {
    enum pubnub_res res;
...
static int printout_subscribe_v2_outcome(pubnub_t* pbp, enum pubnub_res res) {
    ...
    if (PNR_OK != res) {
        printf(
            "Subscribe failed, result=%d('%s')\n",
            res,
            pubnub_res_2_string(res));
        return -1;
    }
    for (msg = pubnub_get_v2(pbp); msg.payload.size > 0;
         msg = pubnub_get_v2(pbp)) {
        puts("Received message:");
        printf(
            "  Channel    = '%.*s'\n",
            (int)msg.channel.size,
            msg.channel.ptr);
        printf("  Timetoken  = '%.*s'\n", (int)msg.tt.size, msg.tt.ptr);
        printf(
            "  Metadata   = '%.*s'\n",
            (int)msg.metadata.size,
            msg.metadata.ptr);
        printf(
            "  Payload    = '%.*s'\n",
...
static int doit(pubnub_t* pbp) {
    ...
    if (PNR_OK != res) {
        printf(
            "First subscribe failed, result=%d('%s')\n",
            res,
            pubnub_res_2_string(res));
        return -1;
    }
