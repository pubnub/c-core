...
static bool valid_bool_input(char const* s) {
    return (s != NULL)
        && (equals_ic(s, "yes") || equals_ic(s, "no") || equals_ic(s, "Y")
            || equals_ic(s, "N"));
}

void pnc_read_bool_from_console_optional(
    char const* message,
    bool* val,
    bool optional) {
    char input[4];
...
void pnc_read_string_from_console_optional(
    char const* message,
    char* val,
    int size,
    bool optional) {
...
void pnc_read_int_from_console_optional(
    char const* message,
    int* val,
    bool optional) {
    char buf[8];
