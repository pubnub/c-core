...
void pnc_ops_subscribe(pubnub_t* pn_sub) {
    ...
    puts(
        "Subscribe will show the first message and return to select operation loop.");
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (strlen(channels_string) == 0 && strlen(groups_string) == 0) {
            puts(
                "You need to add some channels or channel groups first. Ignoring");
            return;
