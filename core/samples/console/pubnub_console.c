        ...
        switch (option) {
            ...
            case MENU_PUBLISH: {
                ...
                while (fgets(message, PNC_READ_STRING_SIZE, stdin) != NULL
                    ...
                    if (message[0] == '{'
                        ...
                        pnc_ops_parse_response(
                            "pubnub_publish()",
                            pubnub_publish(pn, channel, message),
                            pn);
                    } else {
                        ...
                        pnc_ops_parse_response(
                            "pubnub_publish()",
                            pubnub_publish(pn, channel, encoded),
                            pn);
                    }

                    puts(
                        "Enter the message for publish. To exit loop enter Q.");
                }
                break;
            }
            ...
            case MENU_HISTORY: {
                ...
                if (count <= 0) {
                    count = 100;
                    puts(
                        "Count value cannot be less than 1. Default value(100) was applied.");
                }
                ...
                pnc_read_bool_from_console(
                    "Include timetokens",
                    &include_token);

                pnc_ops_parse_response(
                    "pubnub_history()",
                    pubnub_history(pn, channel, count, include_token),
                    pn);
                break;
            }
            ...
            case MENU_HERE_NOW:
                ...
                pnc_ops_parse_response(
                    "pubnub_here_now()",
                    pubnub_here_now(pn, channel, NULL),
                    pn);
                break;
            case MENU_TIME:
            ...
            case MENU_STATE_GET:
                pnc_read_string_from_console(
                    ...
                    channel,
                    PNC_CHANNEL_NAME_SIZE);
                pnc_read_string_from_console_optional(
                    "UUID",
                    user_id,
                    PNC_UUID_SIZE,
                    true);
                ...
                pnc_ops_parse_response(
                    "pubnub_get_state()",
                    pubnub_state_get(pn, channel, NULL, user_id),
                    pn);
                break;
            case MENU_SET_STATE:
                // Set State
                pnc_read_string_from_console(
                    ...
                    PNC_CHANNEL_NAME_SIZE);
                pnc_read_string_from_console(
                    "JSON state without escaping",
                    state,
                    PNC_READ_STRING_SIZE);
                pnc_ops_parse_response(
                    "pubnub_set_state()",
                    pubnub_set_state(
                        pn,
            ...
            case MENU_WHERE_NOW:
                ...
                pnc_ops_parse_response(
                    "pubnub_where_now()",
                    pubnub_where_now(pn, user_id),
                    pn);
                break;
            case MENU_ADD_PRESENCE_CHANNEL: {
                char presence_channel
                    [PNC_CHANNEL_NAME_SIZE + sizeof(PNC_PRESENCE_SUFFIX)];
            ...
            case MENU_GROUP_ADD_CHANNEL:
                ...
                pnc_ops_parse_response(
                    ...
                    pubnub_add_channel_to_group(pn, channel, channel_group),
                    pn);
            ...
            case MENU_GROUP_LIST_CHANNELS:
                ...
                pnc_ops_parse_response(
                    ...
                    pubnub_list_channel_group(pn, channel_group),
                    pn);
...
static void displayMenuOptions(pubnub_t* pn) {
    ...
    printf(
        "Active Channels/Channel Groups are: '%s'/'%s'\n\n",
        channels_string,
        channel_groups_string);
    ...
    printf(
        "ENTER " STRINGIFY(
            MENU_AUTH) "  FOR Setting/Unsetting auth key (current: %s)\n",
        pubnub_auth_get(pn));
    printf(
        "ENTER " STRINGIFY(MENU_UUID) "  FOR Setting UUID (current: %s)\n",
        pubnub_user_id_get(pn));
    ...
    puts("ENTER " STRINGIFY(
        MENU_ADD_PRESENCE_CHANNEL) " FOR [Current Subscription] Add Presence Channel");
    puts("ENTER " STRINGIFY(
        MENU_SUBSCRIBE) " FOR [Current Subscription] Subscribe");
    puts("ENTER " STRINGIFY(
        MENU_UNSUBSCRIBE) " FOR [Current Subscription] Unsubscribe");
    puts("ENTER " STRINGIFY(
        MENU_SUBSCRIPTION_ADD_CHANNEL) " FOR [Current Subscription] Add Channel");
    puts("ENTER " STRINGIFY(
        MENU_SUBSCRIPTION_ADD_GROUP) " FOR [Current Subscription] Add Channel Group");
    puts("ENTER " STRINGIFY(
        MENU_SUBSCRIPTION_REMOVE_CHANNEL) " FOR [Current Subscription] Remove Channel");
    puts("ENTER " STRINGIFY(
        MENU_SUBSCRIPTION_REMOVE_GROUP) " FOR [Current Subscription] Remove Channel Group");
    puts("ENTER " STRINGIFY(
        MENU_GROUP_ADD_CHANNEL) " FOR [Channel Group] Add Channel");
    puts("ENTER " STRINGIFY(
        MENU_GROUP_REMOVE_CHANNEL) " FOR [Channel Group] Remove Channel");
    puts("ENTER " STRINGIFY(
        MENU_GROUP_LIST_CHANNELS) " FOR [Channel Group] List Channels");
    puts("ENTER " STRINGIFY(
        MENU_GROUP_REMOVE_GROUP) " FOR [Channel Group] Remove Group");
