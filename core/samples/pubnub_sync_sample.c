...
static void sync_sample_free(pubnub_t* p)
{
    ...
    if (pubnub_free(p) != 0) { printf("Failed to free the Pubnub context\n"); }
}
...
static int do_time(pubnub_t* pbp)
{
    ...
    puts("-----------------------");
    res = pubnub_time(pbp);
    if (PNR_STARTED == res) { res = pubnub_await(pbp); }
    if (PNR_OK == res) {
        printf(
