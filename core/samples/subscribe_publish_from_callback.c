...
void subscribe_callback(
    ...
    switch (trans) {
        case PBTT_SUBSCRIBE:
            paint_text_yellow();
            printf(
                "%sSubscribe callback, result: %d('%s')\n",
                (char*)user_data,
                result,
            ...
            if (result == PNR_STARTED) {
                paint_text_red();
                printf(
                    "%sSubscribe callback, unexpected: PNR_STARTED(%d)\n",
                    (char*)user_data,
                    result);
                m_stop = 1;
                return;
            ...
            if (PNR_OK == result) {
                ...
                paint_text_red();
                printf(
                    "%sSubscribing failed with code: %d('%s')\n",
                    (char*)user_data,
                    result,
                    pubnub_res_2_string(result));
            }
            if (!m_stop && (result != PNR_CANCELLED)) {
                ...
                if (result != PNR_STARTED) {
                    paint_text_red();
                    printf(
                        "%spubnub_subscribe() returned unexpected: %d('%s')\n",
                        (char*)user_data,
                        result,
                        pubnub_res_2_string(result));
                    m_stop = 1;
        ...
        default:
            ...
            m_stop = 1;
            printf(
                "%sUnexpected transaction tipe:%d - callback(subscribe) result: %d('%s')\n",
                (char*)user_data,
                trans,
                result,
                pubnub_res_2_string(result));
    ...
    switch (trans) {
        case PBTT_PUBLISH:
            ...
            if (result == PNR_STARTED) {
                paint_text_red();
                printf(
                    "%sawait() returned unexpected: PNR_STARTED(%d)\n",
                    (char*)user_data,
                    result);
                m_stop = 1;
                return;
            ...
            if (PNR_OK == result) {
                paint_text_green();
                printf(
                    "%sPublished! Response from Pubnub: %s\n",
                    (char*)user_data,
                    pubnub_last_publish_result(pb));
            } else if (PNR_PUBLISH_FAILED == result) {
                paint_text_red();
                printf(
                    "%sPublished failed on Pubnub, description: %s\n",
                    (char*)user_data,
                    pubnub_last_publish_result(pb));
            } else {
                paint_text_red();
                printf(
                    "%sPublishing failed with code:%d('%s')\n",
                    (char*)user_data,
                    result,
                    pubnub_res_2_string(result));
            }
            if (result != PNR_CANCELLED) {
                ...
                if (result != PNR_STARTED) {
                    paint_text_red();
                    printf(
                        "%spubnub_publish() returned unexpected: %d('%s')\n",
                        (char*)user_data,
                        result,
                        pubnub_res_2_string(result));
                    m_stop = 1;
        ...
        default:
            ...
            m_stop = 1;
            printf(
                "%sUnexpected transaction type:%d callback(publish): result: %d('%s')\n",
                (char*)user_data,
                trans,
                result,
                pubnub_res_2_string(result));
...
int main() {
    ...
    if (pubnub_dns_read_system_servers_ipv4(o_ipv4, 3) > 0) {
        ...
        paint_text_yellow();
        printf(
            "Failed to read system DNS server, will use default %s\n",
    ...
    if (!m_stop) {
        ...
        if (res != PNR_STARTED) {
            printf(
                "pubnub_publish1() returned unexpected: %d('%s')\n",
                res,
