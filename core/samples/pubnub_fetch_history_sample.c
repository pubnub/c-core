...
static void publish_on_channels(pubnub_t* pbp) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        res = pubnub_publish(pbp, m_channel1, msg);
        if (res == PNR_STARTED) {
            puts("Await publish");
    ...
    for (int i = 0; i < 10; i++) {
        ...
        char msg[100];
        sprintf(msg, "\"Hello world from fetch history sample %d !\"", i);
        res = pubnub_publish(pbp, m_channel2, msg);
        if (res == PNR_STARTED) {
            puts("Await publish");
            res = pubnub_await(pbp);
        }
...
int main(int argc, char* argv[]) {
    ...
    if (PNR_OK == res) {
        printf(
            "Got response for Fetch History! Response from Pubnub: %s\n",
            pubnub_get_fetch_history(pbp).ptr);
    } else {
        printf(
            "pubnub_fetch_history() failed with code: %d('%s')\n",
            res,
            pubnub_res_2_string(res));
    }
