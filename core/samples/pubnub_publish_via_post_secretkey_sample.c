...
static struct pubnub_publish_options
publish_opts_method(enum pubnub_method method) {
 ...
 */
static int alloc_and_start_publish_via_post(
    pubnub_t* pb,
    char const* channel,
    char** allocated,
...
int main() {
    ...
    char* allocated;
    char message_via_post_for_gzip[] =
        "\"Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample! - Hello world from publish via post(with GZIP) sample!\"";
    ...
    */
    ...
    time(&t0);
    if (-1 == alloc_and_start_publish_via_post(pbp, chan, &allocated, &res)) {
    ...
    if (PNR_OK == res) {
        ...
        printf(
            "Publishing failed on Pubnub, description: %s\n",
            pubnub_last_publish_result(pbp));
    } else {
        printf(
    ...
    printf(
        "Publish 'via post with gzip' lasted %lf seconds.\n",
        difftime(time(NULL), t0));
    if (PNR_OK == res) {
        printf(
            "Published! Response from Pubnub: %s\n",
