...
struct pbcc_context {
    ...
    */
    ...
    #define APPEND_URL_LITERAL_M_IMP(pbc, string_literal) \
        do { \
            if ((pbc)->http_buf_len + sizeof(string_literal) \
                > sizeof(pbc)->http_buf) { \
                PUBNUB_LOG_ERROR( \
                    "Error: Request buffer too small - cannot append url literal:\n" \
    ...
    #define APPEND_URL_LITERAL_M(pbc, string_literal) \
        APPEND_URL_LITERAL_M_IMP(pbc, "" string_literal)
        ...
        do { \
            ...
            if (M_s_ != NULL) { \
                ...
                if (M_pbc_->http_buf_len + M_n_ > sizeof M_pbc_->http_buf) { \
                    PUBNUB_LOG_ERROR( \
                        "Error: Request buffer too small - cannot append url string:\n" \
                        "current_buffer_size = %lu\n" \
                        "required_buffer_size = %lu\n", \
                ...
                M_pbc_->http_buf_len += snprintf( \
                    M_pbc_->http_buf + M_pbc_->http_buf_len, \
    ...
    #define APPEND_URL_PARAM_TRIBOOL_SIMBOL_M(pbc, name, var, separator) \
        ...
        #define CHECK_IF_GZIP_COMPRESSED(pbc, message) \
            if ((pbc)->gzip_msg_len != 0) { \
                ...
                (pbc)->message_to_send = strcpy( \
                    (pbc)->http_buf + (pbc)->http_buf_len + 1, \
                    (message)); \
            }
    #else
        ...
        #define CHECK_IF_GZIP_COMPRESSED(pbc, message) \
            (pbc)->message_to_send = \
                strcpy((pbc)->http_buf + (pbc)->http_buf_len + 1, (message))
    #endif /* PUBNUB_USE_GZIP_COMPRESSION */

    #define APPEND_MESSAGE_BODY_M(rslt, pbc, message) \
        if ((PNR_OK == (rslt)) && ((message) != NULL)) { \
            if (NOT_COMPRESSED_AND(pbc)( \
                    pb_strnlen_s(message, PUBNUB_MAX_OBJECT_LENGTH) \
                    > sizeof(pbc)->http_buf - (pbc)->http_buf_len - 2)) { \
                PUBNUB_LOG_ERROR( \
                    "Error: Request buffer too small - cannot pack the message body:\n" \
                    "current_buffer_size = %lu\n" \
                    "required_buffer_size = %lu\n", \
                    (unsigned long)(sizeof(pbc)->http_buf), \
                    (unsigned long)((pbc)->http_buf_len + 2 + pb_strnlen_s(message, PUBNUB_MAX_OBJECT_LENGTH))); \
                return PNR_TX_BUFF_TOO_SMALL; \
            } \
    ...
    #define ADD_URL_AUTH_PARAM(pb, name, key) \
        if (pb->auth_token != NULL) { \
            ADD_URL_PARAM(name, key, pb->auth_token); \
        } else if (pb->auth != NULL) { \
            ADD_URL_PARAM(name, key, pb->auth); \
        }
    ...
    #define SORT_URL_PARAMETERS(name) \
        ...
        for (int i = 0; i < name##_count - 1; i++) { \
            for (int j = 0; j < name##_count; j++) { \
                if (name[i].param_key != NULL && name[j].param_key != NULL \
