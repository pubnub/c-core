...
enum pubnub_res pbcc_find_objects_id(
    ...
    if ((*obj != '{') || (*(elem.end++) != '}')) {
        PUBNUB_LOG_ERROR(
            "%s:%d: pbcc_find_objects_id(pbcc=%p) - "
    ...
    if (json_rslt != jonmpOK) {
        PUBNUB_LOG_ERROR(
            "%s:%d: pbcc_find_objects_id(pbcc=%p) - Invalid param: "
...
enum pubnub_res pbcc_getall_uuidmetadata_prep(
    ...
    if (include) {
        ADD_URL_PARAM(qparam, include, include);
    }
    ...
    PUBNUB_LOG_DEBUG(
        "pbcc_getall_uuidmetadata_prep. REQUEST =%s\n",
        pb->http_buf);
...
enum pubnub_res pbcc_remove_uuidmetadata_prep(
    ...
    PUBNUB_LOG_DEBUG(
        "pbcc_remove_uuidmetadata_prep. REQUEST =%s\n",
        pb->http_buf);
    return (rslt != PNR_OK) ? rslt : PNR_STARTED;
}
...
enum pubnub_res pbcc_getall_channelmetadata_prep(
    ...
    PUBNUB_LOG_DEBUG(
        "pbcc_getall_channelmetadata_prep. REQUEST =%s\n",
        pb->http_buf);
    return (rslt != PNR_OK) ? rslt : PNR_STARTED;
}
...
enum pubnub_res pbcc_set_channelmetadata_prep(
    ...
    if (NULL == channel_metadataid) {
        PUBNUB_LOG_ERROR(
            "pbcc_set_channelmetadata_prep(pbcc=%p) - Invalid param: "
    ...
    PUBNUB_LOG_DEBUG(
        "pbcc_set_channelmetadata_prep. REQUEST =%s\n",
        pb->http_buf);
    return (rslt != PNR_OK) ? rslt : PNR_STARTED;
}
...
enum pubnub_res pbcc_get_channelmetadata_prep(
    ...
    if (NULL == channel_metadataid) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_channelmetadata_prep(pbcc=%p) - Invalid param: "
            "channel_metadataid=NULL\n",
            pb);
    ...
    PUBNUB_LOG_DEBUG(
        "pbcc_get_channelmetadata_prep. REQUEST =%s\n",
        pb->http_buf);
    return (rslt != PNR_OK) ? rslt : PNR_STARTED;
}
...
enum pubnub_res pbcc_remove_channelmetadata_prep(
    ...
    if (NULL == channel_metadataid) {
        PUBNUB_LOG_ERROR(
            "pbcc_remove_channelmetadata_prep(pbcc=%p) - Invalid param: "
            "channel_metadataid=NULL\n",
            pb);
    ...
    PUBNUB_LOG_DEBUG(
        "pbcc_remove_channelmetadata_prep. REQUEST =%s\n",
        pb->http_buf);
    return (rslt != PNR_OK) ? rslt : PNR_STARTED;
}
...
enum pubnub_res pbcc_get_members_prep(
    ...
    if (NULL == channel_metadataid) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_members_prep(pbcc=%p) - Invalid param: "
            "channel_metadataid=NULL\n",
            pb);
...
enum pubnub_res pbcc_set_members_prep(
    ...
    if (NULL == channel_metadataid) {
        PUBNUB_LOG_ERROR(
            "pbcc_set_members_prep(pbcc=%p) - Invalid param: "
            "channel_metadataid=NULL\n",
            pb);
...
enum pubnub_res pbcc_parse_objects_api_response(struct pbcc_context* pb) {
    ...
    if ((*reply != '{') || (*(elem.end++) != '}')) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_objects_api_response(pbcc=%p) - Invalid: "
    ...
    if (pbjson_value_for_field_found(&elem, "status", "403")) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_objects_api_response(pbcc=%p) - AccessDenied: "
    ...
    if (jonmpKeyNotFound == json_rslt) {
        ...
        if (json_rslt != jonmpOK) {
            PUBNUB_LOG_ERROR(
                "pbcc_parse_objects_api_response(pbcc=%p) - Invalid response: "
        ...
        PUBNUB_LOG_WARNING(
            "pbcc_parse_objects_api_response(pbcc=%p): \"error\"='%.*s'\n",
            pb,
    ...
    PUBNUB_LOG_TRACE(
        "pbcc_parse_objects_api_response(pbcc=%p): \"data\"='%.*s'\n",
        pb,
        (int)(parsed.end - parsed.start),
        parsed.start);
