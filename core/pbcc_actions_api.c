...
enum pubnub_res pbcc_form_the_action_object(
    ...
    if (NULL == user_id) {
        PUBNUB_LOG_ERROR(
            "pbcc_form_the_action_object(pbcc=%p) - user_id not set.\n",
            pb);
    ...
    if (('\"' != **val)
        ...
        PUBNUB_LOG_ERROR(
            "pbcc_form_the_action_object(pbcc=%p) - "
    ...
    if (buffer_size < sizeof("{\"type\":\"\",\"value\":,\"user_id\":\"\"}")
        ...
        PUBNUB_LOG_ERROR(
            ...
            (unsigned long)(sizeof("{\"type\":\"\",\"value\":,\"user_id\":\"\"}") + pb_strnlen_s(type_literal, sizeof "reaction") + pb_strnlen_s(*val, PUBNUB_MAX_OBJECT_LENGTH) + pb->user_id_len));
...
pubnub_chamebl_t pbcc_get_message_timetoken(struct pbcc_context* pb) {
    ...
    if (pb->last_result != PNR_OK) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_message_timetoken(pb=%p) can be called only if "
    ...
    if (json_rslt != jonmpOK) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_message_timetoken(pbcc=%p) - Invalid response: "
...
pubnub_chamebl_t pbcc_get_action_timetoken(struct pbcc_context* pb) {
    ...
    if (pb->last_result != PNR_OK) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_action_timetoken(pb=%p) can be called only if "
    ...
    if (json_rslt != jonmpOK) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_action_timetoken(pbcc=%p) - Invalid response: "
...
enum pubnub_res pbcc_remove_action_prep(
    ...
    if ((*message_timetoken.ptr != '\"')
        ...
        PUBNUB_LOG_ERROR(
            "pbcc_remove_action_prep(pbcc=%p) - "
...
#endif
    if (start) {
        ADD_URL_PARAM(qparam, start, start);
    }
    if (end) {
        ADD_URL_PARAM(qparam, end, end);
    }
    if (limit != 0) {
        ADD_URL_PARAM_SIZET(qparam, limit, limit);
    }
...
enum pubnub_res pbcc_get_actions_more_prep(struct pbcc_context* pb) {
    ...
    if (pb->last_result != PNR_OK) {
        PUBNUB_LOG_ERROR(
            "pbcc_get_actions_more_prep(pb=%p) can be called only if "
    ...
    if (jonmpKeyNotFound == json_rslt) {
        ...
        PUBNUB_LOG_ERROR(
            "pbcc_get_actions_more_prep(pbcc=%p) - Invalid response: "
...
enum pubnub_res pbcc_history_with_actions_prep(
    ...
    PUBNUB_LOG_DEBUG(
        "pbcc_history_with_actions_prep. REQUEST =%s\n",
        pb->http_buf);
    return (rslt != PNR_OK) ? rslt : PNR_STARTED;
}
...
enum pubnub_res pbcc_parse_actions_api_response(struct pbcc_context* pb) {
    ...
    if ((*reply != '{') || (*(elem.end++) != '}')) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_actions_api_response(pbcc=%p) - Invalid: "
    ...
    if (pbjson_value_for_field_found(&elem, "status", "403")) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_actions_api_response(pbcc=%p) - AccessDenied: "
    ...
    if (jonmpKeyNotFound == json_rslt) {
        ...
        if (json_rslt != jonmpOK) {
            PUBNUB_LOG_ERROR(
                "pbcc_parse_actions_api_response(pbcc=%p) - Invalid response: "
        ...
        PUBNUB_LOG_WARNING(
            "pbcc_parse_actions_api_response(pbcc=%p): \"error\"='%.*s'\n",
            pb,
    ...
    PUBNUB_LOG_TRACE(
        "pbcc_parse_actions_api_response(pbcc=%p): \"data\"='%.*s'\n",
        pb,
        (int)(parsed.end - parsed.start),
        parsed.start);
...
enum pubnub_res
pbcc_parse_history_with_actions_response(struct pbcc_context* pb) {
    char const* reply = pb->http_reply;
    int replylen = pb->http_buf_len;
    struct pbjson_elem elem;
    struct pbjson_elem parsed;
    ...
    if ((*reply != '{') || (*(elem.end++) != '}')) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_history_with_actions_response(pbcc=%p) - Invalid: "
            "response from server is not JSON - response='%s'\n",
            pb,
            reply);
    ...
    if (pbjson_value_for_field_found(&elem, "status", "403")) {
        PUBNUB_LOG_ERROR(
            "pbcc_parse_history_with_actions_response(pbcc=%p) - AccessDenied: "
            "response from server - response='%s'\n",
            pb,
            reply);
        return PNR_ACCESS_DENIED;
    ...
    if (jonmpKeyNotFound == json_rslt) {
        ...
        if (json_rslt != jonmpOK) {
            PUBNUB_LOG_ERROR(
                "pbcc_parse_history_with_actions_response(pbcc=%p) - Invalid response: "
        ...
        PUBNUB_LOG_WARNING(
            "pbcc_parse_history_with_actions_response(pbcc=%p): "
            "\"error_message\"='%.*s'\n",
            pb,
            (int)(parsed.end - parsed.start),
            parsed.start);
    ...
    PUBNUB_LOG_TRACE(
        "pbcc_parse_history_with_actions_response(pbcc=%p): \"channels\"='%.*s'\n",
        pb,
        (int)(parsed.end - parsed.start),
        parsed.start);
