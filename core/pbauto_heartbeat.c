...
static void add_heartbeat_in_progress(unsigned thumper_index) {
    pubnub_mutex_lock(m_watcher.mutw);
    m_watcher
        .heartbeat_in_progress_index_array[m_watcher.heartbeats_in_progress++] =
        thumper_index;
    pubnub_mutex_unlock(m_watcher.mutw);
}
#endif
...
static void heartbeat_thump(pubnub_t* pb, pubnub_t* heartbeat_pb) {
    ...
    if (((channel != NULL)
        ...
        if ((res != PNR_STARTED) && (res != PNR_OK)) {
            PUBNUB_LOG_ERROR(
                "heartbeat_thump(pb=%p, heartbeat_pb) - "
...
static void auto_heartbeat_callback(
    ...
    if (PNR_OK == result) {
        ...
        if (result != PNR_CANCELLED) {
            PUBNUB_LOG_WARNING(
                "punbub_heartbeat(heartbeat_pb=%p) failed with "
                "code: %d('%s') - "
                "will try again.\n",
                heartbeat_pb,
                result,
...
static void handle_heartbeats_in_progress(void) {
    ...
    for (i = 0; i < m_watcher.heartbeats_in_progress;) {
        pubnub_t* heartbeat_pb =
            heartbeat_data[heartbeat_indexes[i]].heartbeat_pb;
        ...
        if (result != PNR_STARTED) {
            ...
            pbauto_take_the_node_out(
                heartbeat_indexes,
                i,
                &m_watcher.heartbeats_in_progress);
...
int pubnub_set_heartbeat_period(pubnub_t* pb, size_t period_sec) {
    ...
    if (UNASSIGNED == pb->thumperIndex) {
        pubnub_mutex_unlock(pb->monitor);
        PUBNUB_LOG_ERROR(
            "Error: pubnub_set_heartbeat_period(pb=%p) - "
...
static enum pubnub_res write_auto_heartbeat_channelInfo(
    ...
    if (channel != NULL) {
        pb->channelInfo.channel =
            strndup(channel, PUBNUB_MAX_OBJECT_LENGTH - 1);
