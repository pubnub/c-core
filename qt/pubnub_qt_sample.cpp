...
void pubnub_qt_sample::onPublish(pubnub_res result) {
    d_out << "onPublish! Result: '" << pubnub_res_2_string(result)
          << "', Response: " << d_pb.last_publish_result() << "\n";
...
void pubnub_qt_sample::onSubscribe(pubnub_res result) {
    d_out << "onSubscribe! Result: '" << pubnub_res_2_string(result)
          << "' Messages:\n";
...
void pubnub_qt_sample::onTime(pubnub_res result) {
    d_out << "onTime! Result: '" << pubnub_res_2_string(result)
          << "' Time:" << d_pb.get() << '\n';
...
void pubnub_qt_sample::onHistory(pubnub_res result) {
    d_out << "onHistory! Result: '" << pubnub_res_2_string(result)
          << "' Messages:\n";
    QList<QString> msg = d_pb.get_all();
    for (int i = 0; i < msg.size(); ++i) {
        d_out << msg[i].toLatin1().data() << '\n';
...
void pubnub_qt_sample::onHereNow(pubnub_res result) {
    d_out << "onHereNow! Result: '" << pubnub_res_2_string(result)
          << "' Response:\n"
          << d_pb.get() << "\n";
...
void pubnub_qt_sample::onWhereNow(pubnub_res result) {
    d_out << "onWhereNow! Result: '" << pubnub_res_2_string(result)
          << "' Response:\n"
          << d_pb.get() << "\n";
...
void pubnub_qt_sample::onSetState(pubnub_res result) {
    d_out << "onSetState! Result: '" << pubnub_res_2_string(result)
          << "' Response:\n"
          << d_pb.get() << "\n";
...
void pubnub_qt_sample::onStateGet(pubnub_res result) {
    d_out << "onStateGet! Result: '" << pubnub_res_2_string(result)
          << "' Response:\n"
          << d_pb.get() << "\n";
    QCoreApplication::instance()->quit();
}
...
void pubnub_qt_sample::execute() {
    ...
    connect(
        &d_pb,
        SIGNAL(outcome(pubnub_res)),
        this,
        SLOT(onPublish(pubnub_res)));
