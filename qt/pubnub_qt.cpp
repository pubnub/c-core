...
bool pubnub_qt::check_if_default_channel_and_groups(
...
void pubnub_qt::auto_heartbeat_prepare_channels_and_ch_groups(
    QString const& channel,
    QString const& channel_group,
    QString& prep_channels,
    QString& prep_channel_groups) {
    if (check_if_default_channel_and_groups(
            channel,
            channel_group,
            prep_channels,
            prep_channel_groups)
...
void pubnub_qt::update_channels_and_ch_groups(
    QString const& channel,
    QString const& channel_group) {
    if (channel.isEmpty() && channel_group.isEmpty()) {
        d_channels.clear();
    ...
    if (channel.isEmpty() == false) {
        remove_from_list(
            d_channels,
            channel.split(',', STRING_SKIP_EMPTY_PARTS));
    }
    if (channel_group.isEmpty() == false) {
        remove_from_list(
            d_channel_groups,
            channel_group.split(',', STRING_SKIP_EMPTY_PARTS));
    }
}
...
void pubnub_qt::auto_heartbeatTimeout() {
    ...
    #if QT_VERSION >= QT_6
        qDebug() << "auto heartbeat(pbcc=" << d_context.get()
                 << ") failed. - result: '" << pubnub_res_2_string(result)
                 << "'\n";
    #else
        qDebug() << "auto heartbeat(pbcc=" << d_context
                 << ") failed. - result: '" << pubnub_res_2_string(result)
                 << "'\n";
...
int pubnub_qt::set_heartbeat_period(size_t period_sec) {
    ...
    d_auto_heartbeat_period_sec = period_sec < PUBNUB_MIN_HEARTBEAT_PERIOD
        ? PUBNUB_MIN_HEARTBEAT_PERIOD
        : period_sec;
    return 0;
}
...
int pubnub_qt::enable_auto_heartbeat(size_t period_sec) {
    ...
    if (!d_auto_heartbeat_enabled) {
        connect(
            d_auto_heartbeatTimer,
            SIGNAL(timeout()),
            this,
            SLOT(auto_heartbeatTimeout()));
...
void pubnub_qt::stop_auto_heartbeat_before_transaction(
    pubnub_trans transaction) {
    KEEP_THREAD_SAFE();
    switch (transaction) {
        case PBTT_HEARTBEAT:
        case PBTT_SUBSCRIBE:
...
void pubnub_qt::start_auto_heartbeat_timer(pubnub_res pbres) {
    ...
    switch (d_trans) {
        ...
        case PBTT_SUBSCRIBE_V2:
            if (PNR_OK == pbres) {
                d_auto_heartbeatTimer->start(
                    d_auto_heartbeat_period_sec * UNIT_IN_MILLI);
            }
            break;
        default:
            break;
...
bool pubnub_qt::is_auto_heartbeat_enabled() {
...
#else
    #define auto_heartbeat_prepare_channels_and_ch_groups( \
        channel, \
        channel_group, \
        prep_channels, \
        prep_channel_groups) \
    ...
    #define check_if_default_channel_and_groups( \
        ...
        prep_channels, \
        prep_channel_groups) \
        auto_heartbeat_prepare_channels_and_ch_groups( \
            (channel), \
            (channel_group), \
            (prep_channels), \
            (prep_channel_groups))
...
pubnub_res
pubnub_qt::startRequest(pubnub_res result, pubnub_trans transaction) {
    ...
    if (PNR_STARTED == result) {
        ...
        QString user_agent(
            GetOsName() + "-Qt" + QT_VERSION_STR + "-PubNub-core/"
            + PUBNUB_SDK_VERSION);
...
void pubnub_qt::set_user_id(QString const& user_id) {
    ...
    pbcc_set_user_id(
        d_context.data(),
        user_id.isEmpty() ? NULL : user_id.toLatin1().data());
}
...
pubnub_res
pubnub_qt::publish_via_post(QString const& channel, QByteArray const& message) {
    QMutexLocker lk(&d_mutex);
...
pubnub_res pubnub_qt::publish_via_post_with_gzip(
    QString const& channel,
...
pubnub_res
pubnub_qt::signal(QString const& channel, QByteArray const& message) {
    KEEP_THREAD_SAFE();
    return startRequest(
        pbcc_signal_prep(
            d_context.data(),
...
pubnub_res
pubnub_qt::subscribe(QString const& channel, QString const& channel_group) {
    QString prep_channels;
    QString prep_channel_groups;
    KEEP_THREAD_SAFE();
    auto_heartbeat_prepare_channels_and_ch_groups(
    ...
    return startRequest(
        pbcc_subscribe_prep(
            ...
            prep_channel_groups.isEmpty()
                ? 0
                : prep_channel_groups.toLatin1().data(),
            0),
        PBTT_SUBSCRIBE);
}

pubnub_res
pubnub_qt::subscribe_v2(QString const& channel, subscribe_v2_options opt) {
    QString prep_channels;
    QString prep_channel_groups;
...
pubnub_res
pubnub_qt::history(QString const& channel, unsigned count, bool include_token) {
    KEEP_THREAD_SAFE();
    return startRequest(
        pbcc_history_prep(
            d_context.data(),
...
pubnub_res
pubnub_qt::message_counts(QString const& channel, QString const& timetoken) {
    KEEP_THREAD_SAFE();
    return startRequest(
        pbcc_message_counts_prep(
            PBTT_MESSAGE_COUNTS,
            d_context.data(),
            channel.isEmpty() ? 0 : channel.toLatin1().data(),
...
pubnub_res pubnub_qt::message_counts(
    QStringList const& channel,
    QString const& timetoken) {
...
pubnub_res pubnub_qt::message_counts(
    QVector<QPair<QString, QString>> const& channel_timetokens) {
    QString ch_list("");
    QString tt_list("");
...
QMap<QString, size_t> pubnub_qt::get_channel_message_counts() {
    ...
    for (int i = 0; i < count; i++) {
        map.insert(
            QString(QByteArray(
                chan_msg_counters[i].channel.ptr,
...
pubnub_res
pubnub_qt::heartbeat(QString const& channel, QString const& channel_group) {
    QString prep_channels;
    QString prep_channel_groups;
    KEEP_THREAD_SAFE();
    auto_heartbeat_prepare_channels_and_ch_groups(
...
pubnub_res
pubnub_qt::here_now(QString const& channel, QString const& channel_group) {
    KEEP_THREAD_SAFE();
    return startRequest(
        pbcc_here_now_prep(
            d_context.data(),
...
pubnub_res pubnub_qt::set_state(
    QString const& channel,
    QString const& channel_group,
...
pubnub_res pubnub_qt::state_get(
    QString const& channel,
    QString const& channel_group,
    QString const& user_id) {
    KEEP_THREAD_SAFE();
...
pubnub_res pubnub_qt::remove_channel_from_group(
    QString const& channel,
    QString const& channel_group) {
    KEEP_THREAD_SAFE();
    return startRequest(
...
pubnub_res pubnub_qt::add_channel_to_group(
    QString const& channel,
    QString const& channel_group) {
    KEEP_THREAD_SAFE();
    return startRequest(
...
pubnub_res
pubnub_qt::get_memberships(QString const& user_id, list_options& options) {
    KEEP_THREAD_SAFE();
    return startRequest(
...
pubnub_res pubnub_qt::set_memberships(
    ...
    d_method = (d_message_to_send.size() != obj.size()) ? pubnubUsePATCHwithGZIP
                                                        : pubnubUsePATCH;
    return startRequest(
...
pubnub_res
pubnub_qt::get_members(QString const& space_id, list_options& options) {
    KEEP_THREAD_SAFE();
    return startRequest(
...
pubnub_res pubnub_qt::add_members(
...
pubnub_res pubnub_qt::update_members(
    QString const& space_id,
    QByteArray const& update_obj,
    QString& include) {
...
pubnub_res pubnub_qt::remove_members(
    QString const& space_id,
    QByteArray const& update_obj,
    QString& include) {
...
#if PUBNUB_USE_ACTIONS_API
pubnub_res pubnub_qt::add_message_action(
    QString const& channel,
    ...
    KEEP_THREAD_SAFE();
    rslt = pbcc_form_the_action_object(
        d_context.data(),
        obj_buffer,
        sizeof obj_buffer,
        actype,
...
pubnub_res pubnub_qt::remove_message_action(
    QString const& channel,
    QString const& message_timetoken,
    QString const& action_timetoken) {
    KEEP_THREAD_SAFE();
    return startRequest(
        pbcc_remove_action_prep(
            d_context.data(),
            channel.toLatin1().data(),
...
pubnub_res pubnub_qt::get_message_actions(
    QString const& channel,
    QString const& start,
    QString const& end,
    size_t limit) {
...
pubnub_res pubnub_qt::get_message_actions_more() {
    KEEP_THREAD_SAFE();
    return startRequest(
        pbcc_get_actions_more_prep(d_context.data()),
        PBTT_GET_ACTIONS);
}

pubnub_res pubnub_qt::history_with_message_actions(
    QString const& channel,
    QString const& start,
    QString const& end,
    size_t limit) {
    KEEP_THREAD_SAFE();
    return startRequest(
        pbcc_history_with_actions_prep(
            d_context.data(),
            channel.toLatin1().data(),
            start.isEmpty() ? NULL : start.toLatin1().data(),
            end.isEmpty() ? NULL : end.toLatin1().data(),
