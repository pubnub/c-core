...
void pubnub_qt_gui_sample::onPublish(pubnub_res result) {
    QString report = QString("Publish result: '") + pubnub_res_2_string(result)
        + "', response: " + d_pb_publish->last_publish_result() + "\n";
...
void pubnub_qt_gui_sample::onSubscribe(pubnub_res result) {
    if (PNR_OK != result) {
        d_console->insertPlainText(
            QString("Subscribe failed, result: ") + pubnub_res_2_string(result)
            + '\n');
    } else {
...
void pubnub_qt_gui_sample::messageChanged() {
    ...
    pubnub_res result =
        d_pb_publish->publish(d_channel->text(), d_message->text());
    if (result != PNR_STARTED) {
        d_console->insertPlainText(
            QString("Publish failed, result: '") + pubnub_res_2_string(result)
            + "'\n");
    }
}
