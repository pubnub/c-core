...
BaseType_t xNetifInit(void) {
    ...
    for (x = 0; x < sizeof(pucEthernetBufferPointers) / sizeof(unsigned char*);
         x++) {
...
static pcap_if_t* prvPrintAvailableNetworkInterfaces(void) {
    ...
    printf(
        "Attempting to open interface number %d.\r\n",
...
static void prvInterruptSimulator(void* pvParameters) {
    ...
    for (;;) {
        /* Get the next packet. */
        lResult =
            pcap_next_ex(pxOpenedInterfaceHandle, &pxHeader, &pucPacketData);
        if (lResult) {
            ...
            if (lLengthOfDataInBuffer[ucNextBufferToFill] == 0L) {
                ...
                memcpy(
                    pucEthernetBufferPointers[ucNextBufferToFill],
                    pucPacketData,
                    pxHeader->len);
