...
BaseType_t xStdoutLoggingUsed = pdFALSE, xDiskFileLoggingUsed = pdFALSE,
...
#if ((ipconfigHAS_DEBUG_PRINTF == 1) || (ipconfigHAS_PRINTF == 1))
    {
        ...
        if ((xStdoutLoggingUsed != pdFALSE)
            ...
            pvLoggingThreadEvent =
                CreateEvent(NULL, FALSE, TRUE, "StdoutLoggingEvent");
...
void vLoggingPrintf(const char* pcFormat, ...) {
    ...
    while ((*pcSource) != '\0') {
        *pcTarget = *pcSource;
        ...
        if ((isxdigit(pcSource[0]) != pdFALSE) && (pcSource[1] == 'i')
            && (pcSource[2] == 'p')) {
            *pcTarget = *pcSource;
            pcTarget++;
            *pcTarget = '\0';
            ...
            while ((pcTarget > pcBegin)
                   && (isxdigit(pcTarget[-1]) != pdFALSE)) {
                pcTarget--;
            }
