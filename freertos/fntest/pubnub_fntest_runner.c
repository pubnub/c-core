...
#define LIST_TEST(tstname) \
    { pnfn_test_##tstname, #tstname, NULL }
...
void test_runner(void* arg) {
    ...
    for (;;) {
        ...
        while (failed_count + passed_count + indete_count < TEST_COUNT) {
            struct TestResultMessage msg;
            if ((tests_in_progress < g_max_conc_tests)
                && (next_test < TEST_COUNT)) {
                start_test(next_test++);
                ++tests_in_progress;
            }
            if (pdTRUE
                == xQueueReceive(m_TestResultQueue, &msg, pdMS_TO_TICKS(20))) {
                switch (msg.result) {
                    case trFail:
                        FreeRTOS_printf(
                            ("\n !!!!!!! The %d. test ('%s') failed!\n\n",
                             msg.test + 1,
                             m_aTest[msg.test].name));
                        ++failed_count;
                        break;
                    case trPass:
                        ++passed_count;
...
void StartPubnubTask(uint16_t usTaskStackSize, UBaseType_t uxTaskPriority) {
    m_TestResultQueue =
        xQueueCreate(g_max_conc_tests + 2, sizeof(struct TestResultMessage));
