...
class msg_on_chan {
  ...
  public:
    ...
    bool operator<(msg_on_chan const& x) const {
        return (d_message == x.d_message) ? (d_channel < x.d_channel)
                                          : (d_message < x.d_message);
    }
};

/// Returns whether the given messages have been received on the context
inline bool
got_messages(context const& pb, std::vector<std::string> const& msg) {
    auto msg_sorted(msg);
...
template<typename T>
std::vector<T>&
inplace_set_difference(std::vector<T>& from, std::vector<T>& to_remove) {
...
bool subscribe_and_check(
    context& pb,
    std::string const& channel,
    const std::string& chgroup,
    std::chrono::milliseconds rel_time,
    std::vector<msg_on_chan> expected);
...
class expect {
    ...
    std::string make_description(char const* op, T expected) {
        ...
        stream << std::boolalpha << "For expression '" << d_expr << "'"
               << std::endl
               << "  in file: " << d_fname << " line: " << d_line;
        if (d_descr) {
            stream << std::endl << "  description: " << d_descr;
        }
        stream << std::endl
               << "  with check: '" << op << "', got: " << d_t
               << ", expected: " << expected;
        return stream.str();
    }
  ...
  public:
    ...
    std::string what() const {
        ...
        stream << "In file:" << d_file << ", line:" << d_line << " "
               << ((0 != *d_expr) ? "For expression:'" : "") << d_expr
               << "' - got:'" << d_str << "'";
        return stream.str();
    }
};

inline except_test make_test_exception(
    std::string describe,
    char const* f,
    long l,
    char const* expr) {
    ...
    #define test_exception(str, fname, line, expr) \
        make_test_exception((str), (fname), (line), (expr))
...
class sense {
  ...
  public:
    ...
    sense& before(std::chrono::milliseconds deadline) {
        return in(deadline);
    }
    sense& operator<<(std::chrono::milliseconds deadline) {
        return in(deadline);
    }
...
class sense_double {
  ...
  public:
    ...
    sense_double& in(std::chrono::milliseconds deadline) {
        ...
        bool trans_finished(
            wait_for(d_left, d_right, deadline, d_result_left, d_result_right));
        if (trans_finished) {
            check_result_for_exceptions(
                d_left,
                d_result_left,
        ...
        expect<bool>(
            ...
            d_fname,
            d_line,
            "waiting for two contexts timed out")
            == true;
        return *this;
    }
    sense_double& before(std::chrono::milliseconds deadline) {
        return in(deadline);
    }
    sense_double& operator<<(std::chrono::milliseconds deadline) {
        return in(deadline);
    }
    ...
    void operator==(pubnub_res expected) {
        ...
        if (d_result_left == d_result_right) {
            ...
            expect<bool>(
                ...
                d_fname,
                d_line,
                "waiting for two contexts failed")
                == true;
        }
    }
};
...
template<typename T>
expect<T> make_expect(T t, char const* e, char const* f, long l) {
    return expect<T>(t, e, f, l);
}
