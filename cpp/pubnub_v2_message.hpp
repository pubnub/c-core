...
class v2_message {
  ...
  public:
    v2_message(struct pubnub_v2_message message_v2) {
        d_ = message_v2;
    }
    v2_message() {
    ...
    std::string tt() const {
        return std::string(d_.tt.ptr, d_.tt.size);
    }
    int region() const {
        return d_.region;
    }
    ...
    std::string channel() const {
        return std::string(d_.channel.ptr, d_.channel.size);
    }
    std::string match_or_group() const {
        return std::string(d_.match_or_group.ptr, d_.match_or_group.size);
    }
    std::string payload() const {
        return std::string(d_.payload.ptr, d_.payload.size);
    }
    std::string metadata() const {
        return std::string(d_.metadata.ptr, d_.metadata.size);
    }
    pubnub_message_type message_type() const {
        return d_.message_type;
    }
    /** Message structure is considered empty if there is no timetoken info(whose
      ...
      */
    bool is_empty() const {
        return (0 == d_.tt.ptr) || (0 == d_.tt.size);
    }
};
