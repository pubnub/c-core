...
static void cpp11(pubnub::context& ipb) {
    ...
    ipb.publish(chan, "\"Hello world from C++!\"")
        .then([=](pubnub::context& pb, pubnub_res res) {
            if (PNR_OK == res) {
                ...
                std::cout << "Publishing failed with code: " << res
                          << std::endl;
            }
            ...
            pb.subscribe(chan).then([=](pubnub::context& pb, pubnub_res res) {
                ...
                pb.subscribe(chan).then([=](pubnub::context& pb,
                                            pubnub_res res) {
                    if (PNR_OK == res) {
                    ...
                    pb.time().then([=](pubnub::context& pb, pubnub_res res) {
                        ...
                        pb.history(chan).then(
                            [=](pubnub::context& pb, pubnub_res res) {
                                if (PNR_OK == res) {
                                    std::cout << "Got history! Messages:"
                                              << std::endl;
                                    auto msg = pb.get_all();
                                    for (auto&& m : msg) {
                                        std::cout << m << std::endl;
                                    ...
                                    std::cout << "Getting history failed!"
                                              << std::endl;
                                }
...
static void on_time(pubnub::context& pb, pubnub_res res) {
    if (PNR_OK == res) {
        std::cout << "Gotten time " << pb.get()
                  << "; last time token=" << pb.last_time_token() << std::endl;
    } else {
        std::cout << "Getting time failed!" << std::endl;
    }
