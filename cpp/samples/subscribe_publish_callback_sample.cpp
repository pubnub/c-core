...
int main() {
    try {
        ...
        if (PNR_STARTED == res) {
            std::cout << "await() returned unexpected: PNR_STARTED"
                      << std::endl;
            return -1;
        }
        if (PNR_OK == res) {
            ...
            std::cout << "Subscribing failed with code: " << (int)res
                      << std::endl;
            ;
        }
        ...
        /* Since the subscribe is ongoing in the `pb` context, we
           ...
           publish
        */
        pubnub::futres fr_2 = pb_2.publish(
            chan,
        ...
        if (PNR_OK == res) {
            ...
            std::cout << "Publishing failed with code: " << (int)res
                      << std::endl;
        }
