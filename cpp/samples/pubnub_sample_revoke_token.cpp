...
int main() {
    try {
        ...
        if (NULL == my_env_publish_key) {
            my_env_publish_key = (char*)"demo";
        }
        if (NULL == my_env_subscribe_key) {
            my_env_subscribe_key = (char*)"demo";
        }
        if (NULL == my_env_secret_key) {
            my_env_secret_key = (char*)"demo";
        }
        ...
        int ttl_minutes = 60;
        char perm_obj[2000];
        char* authorized_uuid =
            "my_authorized_uuid";  // TODO: @reviewers should we change uuid in perm_obj?
        sprintf(
            perm_obj,
            "{\"ttl\":%d, \"uuid\":\"%s\", \"permissions\":{\"resources\":{\"channels\":{ \"mych\":31, \"hello_world\":%d }, \"groups\":{ \"mycg\":31, \"channel-group\":%d }, \"users\":{ \"myuser\":31 }, \"spaces\":{ \"myspc\":31 }}, \"patterns\":{\"channels\":{ }, \"groups\":{ }, \"users\":{ \"^$\":1 }, \"spaces\":{ \"^$\":1 }},\"meta\":{ }}}",
            ttl_minutes,
            authorized_uuid,
            perm_hello_world,
            perm_channel_group);
        ...
        */
        ...
        pubnub::futres futres1 = pb.publish_encrypted(
            chan,
            "\"Hello world from C++!\"",
            "KUKUMENEGRDNA");
#else
        ...
        if (PNR_OK == res) {
            std::cout
                << "Access token permissions revoked! Response from Pubnub: "
                << gb.get_revoke_token_result() << std::endl;
        } else {
            std::cout << "revoke_token() failed with code: " << res
                      << std::endl;
            return 0;
        }
        ...
        pubnub::futres futres2 = pb.publish_encrypted(
            chan,
            "\"Hello world from C++!\"",
            "KUKUMENEGRDNA");
#else
        ...
        if (PNR_OK == res) {
            std::cout
                << "Unexpected publish success (request should fail)! Response from Pubnub: "
                << pb.last_publish_result() << std::endl;
        } else if (PNR_PUBLISH_FAILED == res) {
            std::cout
                << "Expected publish failure on Pubnub as expected, description: "
                << pb.last_publish_result() << std::endl;
        } else {
            std::cout << "Expected publish failure with code: " << res
                      << std::endl;
        }
