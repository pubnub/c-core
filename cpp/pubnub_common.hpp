...
class subscribe_v2_options {
  ...
  public:
    subscribe_v2_options() {
        d_ = pubnub_subscribe_v2_defopts();
    }
    ...
    subscribe_v2_options&
    channel_group(std::vector<std::string> const& chgroup) {
        return channel_group(join(chgroup));
    }
    ...
    pubnub_subscribe_v2_options data() {
        return d_;
    }
};
...
class set_state_options {
  ...
  public:
    ...
    set_state_options& channel_group(std::string const& channel_group) {
        ...
        d_.channel_group =
            d_channel_group.empty() ? 0 : d_channel_group.c_str();

        return *this;
    }

    set_state_options&
    channel_group(std::vector<std::string> const& channel_groups) {
...
class include_options {
  ...
  public:
    ...
    char const**
    include_to_c_strings_array(std::vector<std::string> const& inc) {
        size_t n = inc.size();
        unsigned i;
...
class list_options {
  ...
  public:
    ...
    char const* end() {
        return (d_end.size() > 0) ? d_end.c_str() : NULL;
    }
  ...
  public:
    ...
    */
    ...
    int set_origin(std::string const& origin) {
        ...
        d_origin = origin;
        return pubnub_origin_set(
            d_pb,
            origin.empty() ? NULL : d_origin.c_str());
    }
    /** Returns the current origin for this context
        @see pubnub_get_origin
     */
    std::string origin() const {
        return pubnub_get_origin(d_pb);
    }
    ...
    void set_auth_token(std::string const& token) {
        ...
        d_auth_token = token;
        pubnub_set_auth_token(
            d_pb,
            token.empty() ? NULL : d_auth_token.c_str());
    }
    ...
    get_all_decrypted(std::string const& cipher_key) const {
        std::vector<std::string> all;
        for (;;) {
            pubnub_bymebl_t mebl =
                pubnub_get_decrypted_alloc(d_pb, cipher_key.c_str());
    ...
    /// commas between them. A helper function.
    futres
    subscribe(std::vector<std::string> const& channel, subscribe_options opt) {
    ...
    futres
    message_counts(std::string const& channel, std::string const& timetoken) {
        return doit(pubnub_message_counts(
            d_pb,
            channel.empty() ? 0 : channel.c_str(),
    ...
    futres
    message_counts(std::vector<std::pair<std::string, std::string>> const&
                       channel_timetokens) {
        std::string ch_list("");
        std::string tt_list("");
    ...
    /// commas between them. A helper function.
    futres
    here_now(std::vector<std::string> const& channel, here_now_options opt) {
    ...
    futres set_state(
        ...
        char const* gr = channel_group.empty() ? 0 : channel_group.c_str();
        return doit(
            pubnub_set_state(d_pb, ch, gr, uuid.c_str(), state.c_str()));
    }
    ...
    futres set_state(
        ...
        char const* ch = channel.empty() ? 0 : channel.c_str();
        return doit(
            pubnub_set_state_ex(d_pb, ch, state.c_str(), options.data()));
    }
    ...
    futres set_uuidmetadata(
        std::string const& uuid_metadataid,
    ...
    futres
    get_uuidmetadata(std::string const& uuid, std::string const& include) {
        return doit(
            pubnub_get_uuidmetadata(d_pb, include.c_str(), uuid.c_str()));
    }
    ...
    futres set_channelmetadata(
        std::string const& channel_metadataid,
        std::string const& include,
    ...
    futres remove_channelmetadata(std::string const& channel_metadataid) {
        return doit(
            pubnub_remove_channelmetadata(d_pb, channel_metadataid.c_str()));
    }
    ...
    futres
    get_memberships(std::string const& metadata_uuid, list_options& options) {
    ...
    futres set_memberships(
        std::string const& metadata_uuid,
    ...
    futres
    get_members(std::string const& channel_metadataid, list_options& options) {
    ...
    int set_proxy_authentication_username_password(
