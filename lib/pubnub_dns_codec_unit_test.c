...
static const char encoded_domain_name[] =
    "\7encoded\3two\6server\6domain\4name\1q\2up\0";
...
static const char label_start_encoded_badly_with_offset_to_itself[] =
    "\4www\300\14";
...
#define append_answer_M(name, type, data, ttl) \
    do { \
        ...
        assert( \
            m_msg_size + length + TYPE_AND_CLASS_FIELDS_SIZE + TTL_FIELD_SIZE \
                + RECORD_DATA_LENGTH_FIELD_SIZE + recordDataLength \
            < BUFFER_LENGTH); \
        memcpy(m_buf + m_msg_size, name, length); \
        set_offset((uint8_t*)(name), length); \
        m_msg_size += length; \
...
Ensure(
    pubnub_dns_codec,
    decodes_response_1_question_3_answers_no_ssl_fallback) {
    /* Resolved Ipv4 address */
...
Ensure(pubnub_dns_codec, makes_valid_DNS_query_request) {
    ...
    attest(
        pbdns_prepare_dns_request(buf, sizeof buf, name, &to_send, dnsA),
        equals(0));
...
Ensure(pubnub_dns_codec, handles_buffer_too_small_for_query_request) {
    ...
    /* Shorter buffer */
    attest(
        pbdns_prepare_dns_request(buf, sizeof buf - 1, name, &to_send, dnsA),
        equals(-1));
...
Ensure(pubnub_dns_codec, handles_name_label_stretch_too_long) {
    /* Server name */
    char const name[] =
        "to.a.CrambambulipanchiLabelStretch_tooLongAnd_Just_a_little_bitLonger.domain";
    uint8_t buf[100];
    int to_send;
    attest(
        pbdns_prepare_dns_request(buf, sizeof buf, name, &to_send, dnsA),
...
Ensure(pubnub_dns_codec, fires_asserts_on_illegal_parameters) {
    ...
    expect_assert_in(
        ...
        "pubnub_dns_codec.c");
    expect_assert_in(
        pbdns_prepare_dns_request(m_buf, 3, NULL, &to_send, dnsA),
        "pubnub_dns_codec.c");
    expect_assert_in(
        pbdns_prepare_dns_request(m_buf, 5, "pubsub.pubnub.com", NULL, dnsA),
        "pubnub_dns_codec.c");
    expect_assert_in(
        pbdns_pick_resolved_addresses(
            NULL,
