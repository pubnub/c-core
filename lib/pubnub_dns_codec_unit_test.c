...
enum dnsQuestionClass { QclassInternet = 1 };

/** Pieces used by the tests **/
static const char
    encoded_domain_name[] = "\7encoded\3two\6server\6domain\4name\1q\2up\0";
...
/* Can be used only for the first question(or answer without questions) in a
   row. (Right after the DNS message header.) */
static const char encoded_label_start_with_offset_to_itself[] = "\3www\300\14";
static const char
    label_start_encoded_badly_with_offset_to_itself[] = "\4www\300\14";
static const char encoded_piece1[]                    = "\3www\300\161";
static const char encoded_piece2[] = "\21originServerLabel\300\277";
static char encoded_piece21[] = "\77[<-------the_Longest_Label_Stretch-------"
                                "63_characters------->]\301\277";
...
#define make_dns_header_M(is_response, question_count, answer_count)           \
    do {                                                                       \
        assert(0 == m_msg_size);                                               \
        /* Flags part of the heder is used in a different manner while testing \
         * response and query.*/                                               \
...
#define append_question_M(name)                                                \
    do {                                                                       \
        ...
        assert(                                                                \
            m_msg_size + length + TYPE_AND_CLASS_FIELDS_SIZE < BUFFER_LENGTH); \
...
#define append_answer_M(name, type, data, ttl)                                 \
    do {                                                                       \
        ...
        assert(                                                                \
            m_msg_size + length + TYPE_AND_CLASS_FIELDS_SIZE +                 \
                TTL_FIELD_SIZE + RECORD_DATA_LENGTH_FIELD_SIZE +               \
                recordDataLength <                                             \
            BUFFER_LENGTH);                                                    \
        memcpy(m_buf + m_msg_size, name, length);                              \
        set_offset((uint8_t*)(name), length);                                  \
...
/* Makes sense calling after inserting extra label pieces into test buffer for
 * decoding*/
...
static void multiple_addresses_reset_counters(
...
Ensure(pubnub_dns_codec, decodes_strange_response_2_questions_3_answers)
{
    ...
    /* Assembling test message(response from DNS server) with 2 questions and 2
       answers. Not very complete though.
    */
    ...
    attest(
        pbdns_pick_resolved_addresses(
            ...
            m_msg_size,
            &o_query_type,
            &resolved_addr_ipv4 IPV6_NULL_ARGUMENT PBDNS_OPTIONAL_PARAMS_BP),
        equals(0));
    attest(
        memcmp(&resolved_addr_ipv4, &key_addr, sizeof resolved_addr_ipv4),
        equals(0));
#if PUBNUB_USE_MULTIPLE_ADDRESSES
    attest(
        memcmp(
            bp.spare_addresses.ipv4_addresses[0].ipv4,
            data,
    ...
    attest(
        memcmp(
            bp.spare_addresses.ipv4_addresses[1].ipv4,
            data_2,
...
Ensure(
    pubnub_dns_codec,
    decodes_name_too_long_for_modules_buffer_ending_with_bad_offset_format)
{
    /* Resolved Ipv4 address */
...
Ensure(
    pubnub_dns_codec,
    decodes_name_too_long_for_modules_buffer_finishing_with_0)
{
    /* Resolved Ipv4 address */
    uint8_t                    data[]             = { 100, 200, 30, 40 };
    struct pubnub_ipv4_address key_addr           = { { 0 } };
...
Ensure(pubnub_dns_codec, handles_response_no_usable_answer)
{
    ...
    attest(
        pbdns_pick_resolved_addresses(
            ...
            &resolved_addr_ipv4 IPV6_NULL_ARGUMENT PBDNS_OPTIONAL_PARAMS_BP),
        equals(-1));
    /* Message doesn't contain complete question name(offset missing(2) and last
       character in label stretch(1))?!
    */
    attest(
        pbdns_pick_resolved_addresses(
            NULL,
    ...
    attest(
        pbdns_pick_resolved_addresses(
            NULL,
            m_buf,
            m_msg_size - sizeof data - TYPE_AND_CLASS_FIELDS_SIZE -
                TTL_FIELD_SIZE - RECORD_DATA_LENGTH_FIELD_SIZE - 1,
            &o_query_type,
            &resolved_addr_ipv4 IPV6_NULL_ARGUMENT PBDNS_OPTIONAL_PARAMS_BP),
        equals(-1));
...
Ensure(
    pubnub_dns_codec,
    handles_label_offset_to_itself_preventing_infinite_loop)
{
    /* Resolved Ipv4 address */
...
Ensure(pubnub_dns_codec, handles_splitted_label_too_long_for_modules_buffer)
{
    ...
    make_dns_header_M(RESPONSE, 0, 2);
    /* Setting Authority and Additional Record counts in DNS header and making
       shure pubnub client analisis ignores them
    */
    ...
    place_encoded_label_piece_M(encoded_piece4);
    printf("------->encoded label formed:\n");
    /* Address with time to live 0 wan't be 'cashed'(, but might be used just
     * once) */
...
/* If there is a badly encoded label(some label stretch shorter or longer than
   it indicates its encoded length - let alone the offset) anywhere within
   response before usable answer, the answer won't be found.
 */
...
Ensure(pubnub_dns_codec, makes_valid_DNS_query_request)
{
    ...
    printf("to_send = %d\n", to_send);
    attest(
        (buf[OFFSET_FLAGS] & m_buf[OFFSET_FLAGS]) ||
            (buf[OFFSET_FLAGS + 1] & m_buf[OFFSET_FLAGS + 1]),
        equals(false));
    attest(
        memcmp(
            buf + OFFSET_QUESTION_COUNT,
...
{
    /* Server name */
    char const name[] = "to.a.CrambambulipanchiLabelStretch_tooLongAnd_Just_a_"
                        "little_bitLonger.domain";
    uint8_t    buf[100];
    int        to_send;
    attest(
        pbdns_prepare_dns_request(NULL, buf, sizeof buf, name, &to_send, dnsA),
...
Ensure(
    pubnub_dns_codec,
    handles_response_RecordType_and_DataLength_mismatch_on_Ipv6_answer)
{
    /* Resolved IpvX address */
    uint8_t           data[]   = { 255, 255, 0, 0, 0 };
    ...
    attest(
        pbdns_pick_resolved_addresses(
            ...
            &o_query_type,
            NULL,
            &resolved_addr_ipv6 PBDNS_OPTIONAL_PARAMS_BP),
        equals(-1));
#if PUBNUB_USE_MULTIPLE_ADDRESSES
    attest(bp.spare_addresses.n_ipv4, equals(0));
    attest(bp.spare_addresses.ipv4_index, equals(0));
...
Ensure(pubnub_dns_codec, handles_response_RecordTypeAAAA_and_RecordTypeA)
{
    ...
    uint8_t data_2[]     = { 0xAB, 0xCD, 0x02, 0x55, 0, 0,    0,    0,
                             0x32, 0,    0,    0,    0, 0x14, 0x15, 0x16 };
    uint8_t data_3[]     = { 0xBA, 0xDC, 0x20, 0xAA, 0, 0,    0,    0,
                             0x23, 0,    0,    0,    0, 0x41, 0x51, 0x61 };
    struct pubnub_ipv4_address resolved_addr_ipv4 = { { 0 } };
    ...
    attest(
        ...
        equals(0));
    attest(
        memcmp(&resolved_addr_ipv6, &key_addr, sizeof resolved_addr_ipv6),
        equals(0));
    attest(
        memcmp(
    ...
    attest(
        memcmp(
            bp.spare_addresses.ipv6_addresses[0].ipv6,
            data_2,
    ...
    attest(
        memcmp(
            bp.spare_addresses.ipv6_addresses[1].ipv6,
            data_3,
            sizeof resolved_addr_ipv6.ipv6),
        equals(0));
...
Ensure(pubnub_dns_codec, handles_response_with_RecordTypeAAAA_no_ssl_fallback)
{
    ...
    uint8_t data_1[]      = { 0x0A, 0x0b, 0x0c, 0x0d, 0, 0,    0,    0,
                              0x0e, 0,    0,    0,    0, 0x0f, 0x1a, 0x1b };
    uint8_t data_2[]      = { 0xAB, 0xCD, 0x02, 0x55, 0, 0,    0,    0,
                              0x32, 0,    0,    0,    0, 0x14, 0x15, 0x16 };
    uint8_t data_3[]      = { 0xBA, 0xDC, 0x20, 0xAA, 0, 0,    0,    0,
                              0x23, 0,    0,    0,    0, 0x41, 0x51, 0x61 };
    ...
    attest(
        ...
        equals(0));
    attest(
        memcmp(&resolved_addr_ipv4.ipv4, data_zeros, sizeof resolved_addr_ipv4),
        equals(0));
    attest(
        memcmp(&resolved_addr_ipv6, &key_addr, sizeof resolved_addr_ipv6),
        equals(0));
...
Ensure(pubnub_dns_codec, fires_asserts_on_illegal_parameters)
{
    ...
    expect_assert_in(
        pbdns_pick_resolved_addresses(
            ...
            m_msg_size,
            &o_query_type,
            NULL IPV6_NULL_ARGUMENT PBDNS_OPTIONAL_PARAMS_BP),
        "pubnub_dns_codec.c");
}
