...
int send_dns_query(
    ...
    switch (dest->sa_family) {
        ...
        default:
            PUBNUB_LOG_ERROR(
                "send_dns_query(socket=%d): invalid address family "
                "dest->sa_family =%uh\n",
                skt,
                dest->sa_family);
            return -1;
    ...
    if (-1
        == pbdns_prepare_dns_request(
            ...
            query_type)) {
        PUBNUB_LOG_ERROR(
            "Couldn't prepare dns request! : #prepared bytes=%d\n",
            to_send);
        return -1;
    }
...
int read_dns_response(
    ...
    struct sockaddr* resolved_addr PBDNS_OPTIONAL_PARAMS_DECLARATIONS) {
    uint8_t buf[8192];
    int msg_size;
    ...
    switch (dest->sa_family) {
        ...
        default:
            PUBNUB_LOG_ERROR(
                "read_dns_response(socket=%d): invalid address family "
                "dest->sa_family =%uh\n",
                skt,
                dest->sa_family);
            return -1;
    ...
    if (pbdns_pick_resolved_addresses(
            buf,
            (size_t)msg_size,
            &addr_ipv4 P_ADDR_IPV6_ARGUMENT PBDNS_OPTIONAL_PARAMS)
        != 0) {
        return -1;
    }
    if (addr_ipv4.ipv4[0] != 0) {
        memcpy(
            &((struct sockaddr_in*)resolved_addr)->sin_addr.s_addr,
            addr_ipv4.ipv4,
        ...
        memcpy(
            ((struct sockaddr_in6*)resolved_addr)->sin6_addr.s6_addr,
            addr_ipv6.ipv6,
