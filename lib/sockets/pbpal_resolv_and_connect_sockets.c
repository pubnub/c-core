...
#define send_dns_query(pb, x, y, z, v) -1
#if PUBNUB_USE_MULTIPLE_ADDRESSES
#define read_dns_response(pb, x, y, z, v, p) -1
#else /* PUBNUB_USE_MULTIPLE_ADDRESSES */
#define read_dns_response(pb, x, y, z) -1
...
PUBNUB_STATIC_ASSERT(
    PUBNUB_MAX_DNS_QUERIES < (1 << SENT_QUERIES_SIZE_IN_BITS),
    PUBNUB_MAX_DNS_QUERIES_is_too_big);
#if PUBNUB_CHANGE_DNS_SERVERS
/** Considering the size of bit field for DNS servers rotations count */
PUBNUB_STATIC_ASSERT(
    PUBNUB_MAX_DNS_ROTATION < (1 << ROTATIONS_COUNT_SIZE_IN_BITS),
...
static bool is_ipv4_mapped_ipv6(const struct sockaddr* addr)
{
    ...
    return (
        memcmp(((struct sockaddr_in6*)addr)->sin6_addr.s6_addr, prefix, 12) ==
        0);
}
...
{
    if (addr->sa_family != AF_INET6) return;
...
static void prepare_port_and_hostname(
    const pubnub_t* pb,
    uint16_t*       p_port,
    char const**    p_origin)
{
    ...
    PUBNUB_ASSERT_OPT(
        (pb->state == PBS_READY) || (pb->state == PBS_WAIT_DNS_SEND));
    *p_port = HTTP_PORT;
#if PUBNUB_USE_SSL
    if (pb->flags.trySSL) {
    ...
    if (pb->port != INITIAL_PORT_VALUE) { *p_port = pb->port; }
    ...
    *p_origin = PUBNUB_ORIGIN;
    ...
    case pbproxyHTTP_CONNECT:
        if (!pb->proxy_tunnel_established) { *p_origin = pb->proxy_hostname; }
    ...
    case pbproxyHTTP_GET:
        ...
        *p_port   = pb->proxy_port;
        ...
        if (pubnub_logger_should_log(pb, PUBNUB_LOG_LEVEL_TRACE)) {
            ...
            PUBNUB_LOG_MAP_SET_STRING(&data, *p_origin, origin)
            PUBNUB_LOG_MAP_SET_NUMBER(&data, (double)*p_port, port)
            pubnub_log_object(
                pb,
...
#ifdef PUBNUB_CALLBACK_API
static void get_default_ipv4_dns_ip(
    pubnub_t*                   pb,
    struct pubnub_ipv4_address* addr)
{
    if (pubnub_dns_read_system_servers_ipv4(pb, addr, 1) != 1)
        inet_pton(AF_INET, PUBNUB_DEFAULT_IPV4_DNS_SERVER, addr);
}

#if PUBNUB_USE_IPV6
static void get_default_ipv6_dns_ip(
    pubnub_t*                   pb,
    struct pubnub_ipv6_address* addr)
{
    if (pubnub_dns_read_system_servers_ipv6(pb, addr, 1) != 1)
    ...
    struct sockaddr*            addr)
{
    ...
    if (AF_INET6 == family) {
        ...
        if ((pubnub_get_dns_primary_server_ipv6(
    ...
    if (AF_INET == family || (AF_INET6 == family && !user_provided_ipv6_dns)) {
        ...
        if ((pubnub_get_dns_primary_server_ipv4(
            ...
            (dns_check->dns_server_check & dns_check->dns_mask)) {
            dns_check->dns_mask <<= 1;
            if ((pubnub_get_dns_secondary_server_ipv4(
                     pb, (struct pubnub_ipv4_address*)p) == -1) ||
                (dns_check->dns_server_check & dns_check->dns_mask)) {
                dns_check->dns_mask <<= 1;
                if (AF_INET == family) {
    ...
    if (AF_INET6 == family &&
        ...
        get_default_ipv6_dns_ip(pb, (struct pubnub_ipv6_address*)pv6);
        addr->sa_family = AF_INET6;

        if (is_ipv4_mapped_ipv6(addr)) remap_ipv6_to_ipv4(addr);
    }
#endif /* PUBNUB_USE_IPV6 */
}
 ...
 *  @param[in,out] addr Decided DNS server for origin DNS query.
 */
static void get_dns_ip(pubnub_t* pb, sa_family_t family, struct sockaddr* addr)
{
#if PUBNUB_USE_IPV6
    bool user_provided_ipv6_dns = false;
    if (AF_INET6 == family) {
        ...
        if (pubnub_get_dns_primary_server_ipv6(
            ...
            pubnub_get_dns_secondary_server_ipv6(
                pb, (struct pubnub_ipv6_address*)pv6->s6_addr);
        }
        user_provided_ipv6_dns = !IN6_IS_ADDR_UNSPECIFIED(pv6);
    }
    // For IPv4 preferred family or uninitialized IPv6 address.
    ...
    if (AF_INET6 == family &&
        (AF_INET == addr->sa_family &&
         0 == ((struct sockaddr_in*)addr)->sin_addr.s_addr)) {
        struct in6_addr* pv6 = &((struct sockaddr_in6*)addr)->sin6_addr;
        get_default_ipv6_dns_ip(pb, (struct pubnub_ipv6_address*)pv6->s6_addr);
        addr->sa_family = AF_INET6;
...
static enum pbpal_resolv_n_connect_result connect_TCP_socket(
    pubnub_t*        pb,
    ...
    const uint16_t   port)
{
    ...
    default:
        ...
        if (pubnub_logger_should_log(pb, PUBNUB_LOG_LEVEL_ERROR)) {
            pubnub_log_value_t data = pubnub_log_value_map_init();
            PUBNUB_LOG_MAP_SET_NUMBER(&data, (double)(long)*skt, socket)
            PUBNUB_LOG_MAP_SET_NUMBER(&data, (double)dest->sa_family, sa_family)
            pubnub_log_object(
                pb,
                PUBNUB_LOG_LEVEL_ERROR,
                PUBNUB_LOG_LOCATION,
                &data,
                "Invalid internet protocol:");
        }
#endif
        return pbpal_connect_failed;
    }
    *skt = socket(dest->sa_family, SOCK_STREAM, IPPROTO_TCP);
    if (SOCKET_INVALID == *skt) { return pbpal_connect_resource_failure; }
#if defined(_WIN32)
    const BOOL enabled =
    ...
    pbpal_set_socket_blocking_io(pb, *skt, options->use_blocking_io);
    socket_disable_SIGPIPE(pb, *skt);
...
static void check_dns_server_error(
    struct pbdns_servers_check* dns_check,
    struct pubnub_flags*        flags)
{
    ...
    const uint16_t port)
{
        ...
        if (spare_addresses->ipv6_index < spare_addresses->n_ipv6) {
            ...
            if (pubnub_logger_should_log(pb, PUBNUB_LOG_LEVEL_TRACE)) {
                ...
                PUBNUB_LOG_MAP_SET_NUMBER(
                    &data, spare_addresses->ipv6_index, current)
                PUBNUB_LOG_MAP_SET_NUMBER(&data, spare_addresses->n_ipv6, total)
                pubnub_log_object(
                    pb,
                    PUBNUB_LOG_LEVEL_TRACE,
                    PUBNUB_LOG_LOCATION,
                    &data,
                    "Spare IPv6 addresses:");
            }
#endif
            ...
            if (spare_addresses->ttl_ipv6[spare_addresses->ipv6_index] - 2 >
                ...
                if (pubnub_logger_should_log(pb, PUBNUB_LOG_LEVEL_TRACE)) {
                    ...
                    PUBNUB_LOG_MAP_SET_NUMBER(
                        &data,
                        (double)(long)(spare_addresses->ttl_ipv6
                                           [spare_addresses->ipv6_index]),
                        ttl)
                    PUBNUB_LOG_MAP_SET_NUMBER(
                        &data,
                        (double)(long)(tt - spare_addresses
                                                ->time_of_the_last_dns_query),
                        elapsed)
                    pubnub_log_object(
                        pb,
                        PUBNUB_LOG_LEVEL_TRACE,
...
#if defined(_WIN32)
                // Complete TCP Keep-Alive configuration if connection
                // established.
    ...
    if ((AF_INET == family || pbpal_connect_failed == rslt) &&
        ...
        if (pubnub_logger_should_log(pb, PUBNUB_LOG_LEVEL_TRACE)) {
            pubnub_log_value_t data = pubnub_log_value_map_init();
            PUBNUB_LOG_MAP_SET_NUMBER(
                &data, spare_addresses->ipv4_index, current)
            PUBNUB_LOG_MAP_SET_NUMBER(&data, spare_addresses->n_ipv4, total)
            pubnub_log_object(
                pb,
                PUBNUB_LOG_LEVEL_TRACE,
                PUBNUB_LOG_LOCATION,
        ...
        if (spare_addresses->ttl_ipv4[spare_addresses->ipv4_index] - 2 >
            ...
            if (pubnub_logger_should_log(pb, PUBNUB_LOG_LEVEL_TRACE)) {
                ...
                PUBNUB_LOG_MAP_SET_NUMBER(
                    &data,
                    (double)(long)(spare_addresses
                                       ->ttl_ipv4[spare_addresses->ipv4_index]),
                    ttl)
                PUBNUB_LOG_MAP_SET_NUMBER(
                    &data,
                    (double)(long)(tt -
                                   spare_addresses->time_of_the_last_dns_query),
                    elapsed)
                pubnub_log_object(
                    pb,
                    PUBNUB_LOG_LEVEL_TRACE,
    ...
    else pbpal_multiple_addresses_reset_counters(spare_addresses);
...
enum pbpal_resolv_n_connect_result pbpal_resolv_and_connect(pubnub_t* pb)
{
    ...
    if (PNA_CALLBACK == pb->api_policy) { // if policy
        ...
        if (AF_INET6 == family && has_ipv6_proxy) {
            struct sockaddr_in6 dest = { 0 };
            dest.sin6_family         = AF_INET6;
            PUBNUB_LOG_TRACE(pb, "Connecting via IPv6 proxy address.");
            memcpy(
                dest.sin6_addr.s6_addr,
        ...
        if (0 != pb->proxy_ipv4_address.ipv4[0] &&
            ...
            struct sockaddr_in dest = { 0 };
            dest.sin_family         = AF_INET;
            PUBNUB_LOG_TRACE(pb, "Connecting via IPv4 proxy address.");
            memcpy(
                &(dest.sin_addr.s_addr),
...
#if PUBNUB_CHANGE_DNS_SERVERS
            get_dns_ip(pb, family, &pb->dns_check, (struct sockaddr*)&dest);
#else  /* PUBNUB_CHANGE_DNS_SERVERS */
            get_dns_ip(pb, family, (struct sockaddr*)&dest);
#endif /* !PUBNUB_CHANGE_DNS_SERVERS */
            /* Store DNS server address for later use in
        ...
        if (error > 0) { return pbpal_resolv_send_wouldblock; }
        ...
        for (int pass = 0; pass < 2; ++pass) {
            ...
            for (it = result; it != NULL; it = it->ai_next) {
                ...
                if (pb->pal.socket == SOCKET_INVALID) { continue; }
#if defined(_WIN32)
                const BOOL enabled =
                    pbccTrue == pb->options.tcp_keepalive.enabled ? TRUE
                                                                  : FALSE;
                (void)setsockopt(
                    pb->pal.socket,
                    SOL_SOCKET,
        ...
        if (NULL == it) { return pbpal_connect_failed; }

        socket_set_rcv_timeout(pb->pal.socket, pb->transaction_timeout_ms);
        socket_disable_SIGPIPE(pb, pb->pal.socket);
...
#endif /* defined(_WIN32) */
    if (error_code != 0) {
        PUBNUB_LOG_ERROR(pb, "Socket connection error code: %d", error_code);
#if PUBNUB_USE_MULTIPLE_ADDRESSES
        PUBNUB_LOG_ERROR(
            pb,
...
void pbpal_set_tcp_keepalive(const pubnub_t* pb)
{
    ...
    alive.keepalivetime =
        (keepalive.time > 0) ? (ULONG)keepalive.time * 1000UL : 0;
    (void)WSAIoctl(
        skt,
        SIO_KEEPALIVE_VALS,
        &alive,
    ...
    if (probes > 0)
        (void)setsockopt(
            skt, IPPROTO_TCP, TCP_KEEPCNT, &probes, sizeof(probes));
#endif
}