...
static void get_default_dns_ip(struct pubnub_ipv4_address* addr) {
    if (pubnub_dns_read_system_servers_ipv4(addr, 1) != 1) {
        ...
        #if PUBNUB_CHANGE_DNS_SERVERS
static void
get_dns_ip(struct pbdns_servers_check* dns_check, struct sockaddr* addr) {
    ...
    if ((pubnub_get_dns_primary_server_ipv4((struct pubnub_ipv4_address*)p)
         == -1)
        || (dns_check->dns_server_check & dns_check->dns_mask)) {
        dns_check->dns_mask <<= 1;
        if ((pubnub_get_dns_secondary_server_ipv4(
                 (struct pubnub_ipv4_address*)p)
             == -1)
            || (dns_check->dns_server_check & dns_check->dns_mask)) {
            dns_check->dns_mask <<= 1;
            #if PUBNUB_USE_IPV6
    ...
    if (AF_INET6 == addr->sa_family) {
        if ((pubnub_get_dns_primary_server_ipv6(
                 (struct pubnub_ipv6_address*)pv6)
             == -1)
            || (dns_check->dns_server_check & dns_check->dns_mask)) {
            dns_check->dns_mask <<= 1;
...
static void
if_no_retry_close_socket(pb_socket_t* skt, struct pubnub_flags* flags) {
...
static enum pbpal_resolv_n_connect_result try_TCP_connect_spare_address(
    ...
    if (spare_addresses->ipv4_index < spare_addresses->n_ipv4) {
        ...
        if (spare_addresses->ttl_ipv4[spare_addresses->ipv4_index] - 2
            ...
            rslt =
                connect_TCP_socket(skt, options, (struct sockaddr*)&dest, port);
            ...
            uint8_t* ipv4 =
                spare_addresses->ipv4_addresses[spare_addresses->ipv4_index]
                    .ipv4;
            PUBNUB_LOG_TRACE(
                "Spare IPv4: %d.%d.%d.%d address expired.\n",
                ipv4[0],
                ipv4[1],
                ipv4[2],
                ipv4[3]);
...
enum pbpal_resolv_n_connect_result pbpal_resolv_and_connect(pubnub_t* pb) {
    ...
    if (0 != pb->proxy_ipv4_address.ipv4[0]) {
        ...
        (0 != pb->proxy_ipv6_address.ipv6[0])
    ...
    if (SOCKET_INVALID == pb->pal.socket) {
        pb->pal.socket = socket(
            ((struct sockaddr*)&dest)->sa_family,
            SOCK_DGRAM,
            IPPROTO_UDP);
    }
    if (SOCKET_INVALID == pb->pal.socket) {
    ...
    for (it = result; it != NULL; it = it->ai_next) {
        ...
        if (connect(pb->pal.socket, it->ai_addr, it->ai_addrlen)
            ...
            if (socket_would_block()) {
                ...
                PUBNUB_LOG_WARNING(
                    "socket connect() failed, will try another "
