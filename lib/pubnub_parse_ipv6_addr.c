...
int pubnub_parse_ipv6_addr(
    pubnub_t*                   pb,
    char const*                 addr,
    struct pubnub_ipv6_address* p)
{
    uint16_t b[8] = { 0 };
    ...
    int colons = 0;
    /* Index of hex number(between two colons) in the array of eight 'unsigned
     * short' members */
    unsigned i = 0;
    ...
    if (strchr(addr, ':') == NULL) {
        PUBNUB_LOG_ERROR(pb, "No colons in the IPv6 address string: %s", addr);
        return -1;
    }
    len              = strlen(addr);
    ...
    while (pos != meeting_position) {
        if (':' == *pos) {
            ...
            if (colons > 7) {
                PUBNUB_LOG_ERROR(
                    pb,
                    "More than 7 colons in the IPv6 address string: %s",
                    addr);
                return -1;
            }
            ...
            if (previous_colon) {
                if (FORWARD == step) {
                    /* Will start reading from the other end of the address
            ...
            PUBNUB_LOG_ERROR(
                pb, "Invalid characters in the IPv6 address string: %s", addr);
            return -1;
        }
        ++hex_digits;
        if (hex_digits > 4) {
    ...
    if ((FORWARD == step) && (colons != 7)) {
        PUBNUB_LOG_ERROR(pb, "IPv6 address string is incomplete: %s", addr);
        return -1;
    }
