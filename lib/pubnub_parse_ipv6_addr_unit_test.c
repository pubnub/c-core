...
Ensure(pubnub_parse_ipv6_addr, parses_some_Ipv6_addresses) {
    ...
    uint8_t key2[16] = {
        ...
        0x0e};
    uint8_t key3[16] =
        {0x26, 0x20, 0x01, 0x19, 0, 0x01, 0, 0x35, 0xaf, 0, 0, 0x35};
    uint8_t key4[16] = {
        0,
        0x0a,
        0,
...
Ensure(pubnub_parse_ipv6_addr, parses_something_thats_not_Ipv6_address) {
    ...
    attest(
        pubnub_parse_ipv6_addr("A1.B2.c3.d4", &resolved_addr_ipv6),
        equals(-1));
    ...
    attest(
        pubnub_parse_ipv6_addr("F234:::37", &resolved_addr_ipv6),
        equals(-1));
    attest(pubnub_parse_ipv6_addr("F234:.37", &resolved_addr_ipv6), equals(-1));
    attest(
        pubnub_parse_ipv6_addr(
            "1B3C:01:0:0:1234:FDCA:37:9032:15",
            &resolved_addr_ipv6),
        equals(-1));
    attest(
