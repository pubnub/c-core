...
{
    int result;
    pubnub_bymebl_t const to_encode = { ENC_INPUT_MED, sizeof ENC_INPUT_MED - 1 };
    ...
    printf("Input `%s`\n"
           "encoded to `%s`\n"
           "expected   `%s`.\n", to_encode.ptr, encoded, ENC_RESULT_MED);
    assert(0 == result);
...
static void do_encode_alloc(void)
{
    ...
    pubnub_bymebl_t const to_encode = { ENC_INPUT_SHORT, sizeof ENC_INPUT_SHORT - 1 };
    
    result = pbbase64_encode_alloc_std(to_encode);
    printf("Input `%s`\n"
           "encoded to `%s`\n"
           "expected   `%s`.\n", to_encode.ptr, result.ptr, ENC_RESULT_SHORT);
    assert(result.ptr != NULL);
    assert(result.size == sizeof ENC_RESULT_SHORT - 1);
    assert(0 == strcmp(result.ptr, ENC_RESULT_SHORT));
    free(result.ptr);
}
...
static void do_decode(void)
{
    ...
    printf("Input `%s`\n"
           "decoded to `%s`\n"
           "expected   `%s`.\n", ENC_RESULT_MED, decoded, ENC_INPUT_MED);
    assert(0 == result);
