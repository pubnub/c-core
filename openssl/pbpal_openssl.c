...
static int pal_init(void) {
    ...
    if (!s_init) {
        ...
        PUBNUB_LOG_TRACE(
            "SSLEAY_VERSION_NUMBER=%lx SSLeay()=%lx "
        ...
        switch (SSL_get_error(ssl, result)) {
            ...
            case SSL_ERROR_WANT_X509_LOOKUP:
                if (!pbms_active(pb->pal.tryconn)
                    || (pbms_elapsed(pb->pal.tryconn)
                        < pb->transaction_timeout_ms)) {
                    PUBNUB_LOG_TRACE(
                        "pb=%p: TLS/SSL_I/O operation should retry\n",
                        pb);
...
enum pubnub_res pbpal_line_read_status(pubnub_t* pb) {
    ...
    */
    for (;;) {
        ...
        if (pb->left == 0) {
            PUBNUB_LOG_ERROR(
                "pbpal_line_read_status(pb=%p): buffer full but "
                "newline not found",
                pb);
            pb->sock_state = STATE_NONE;
...
void pbpal_free(pubnub_t* pb) {
    ...
    if (pb->pal.socket != SOCKET_INVALID) {
        pbntf_lost_socket(pb);
        PUBNUB_LOG_TRACE(
            "pbpal_free(%p): Unexpected pb->pal.socket == %d\n",
            pb,
            pb->pal.socket);
        socket_close(pb->pal.socket);
        pb->pal.socket = SOCKET_INVALID;
