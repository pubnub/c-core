    ...
    pubnub_bymebl_t* encrypted)
{
    ...
    if (!EVP_EncryptInit_ex(aes256, EVP_aes_256_cbc(), NULL, key, iv)) {
        /* ERR_print_errors_cb - no pubnub_t* context available */
...
pubnub_bymebl_t pbaes256_encrypt_alloc(
    pubnub_bymebl_t msg,
    uint8_t const*  key,
    uint8_t const*  iv)
{
    ...
    result.ptr = (uint8_t*)malloc(
        msg.size + EVP_CIPHER_block_size(EVP_aes_256_cbc()) * 2);
#else
    result.ptr = (uint8_t*)malloc(
        msg.size + EVP_CIPHER_block_size(EVP_aes_256_cbc()));
#endif

    if (NULL == result.ptr) {
        EVP_CIPHER_CTX_free(aes256);
        /* PUBNUB_LOG_ERROR("Failed to allocate memory for AES-256
         * encryption\n"); */
        return result;
    }
    ...
    pubnub_bymebl_t* msg)
{
    ...
    if (!EVP_DecryptInit_ex(
            aes256, EVP_aes_256_cbc(), NULL, key, terminated_iv)) {
        /* ERR_print_errors_cb - no pubnub_t* context available */
    ...
    uint8_t const*  iv)
{
    ...
    if (NULL == result.ptr) { return result; }
