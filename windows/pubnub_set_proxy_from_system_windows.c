    ...
    if (use_auto_proxy) {
        ...
        if (NULL != winhttp) {
            if (autoproxy_opts.lpszAutoConfigUrl != NULL) {
                ...
                autoproxy_opts.dwAutoDetectFlags = WINHTTP_AUTO_DETECT_TYPE_DHCP
            ...
            if (WinHttpGetProxyForUrl(
                ...
                if (proxy_info.lpszProxy != NULL) {
                    ...
                    PUBNUB_LOG_INFO(
                        "Auto detected proxy: %S\n",
                        proxy_info.lpszProxy);
                }
            } else {
                PUBNUB_LOG_WARNING(
                    "Cannot auto detect proxy, "
