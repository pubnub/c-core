...
{
    if (family == AF_INET) return is_valid_ipv4(addr);
#if PUBNUB_USE_IPV6
    if (family == AF_INET6) return is_valid_ipv6(addr);
#endif /* PUBNUB_USE_IPV6 */
    return false;
}
...
static bool is_adapter_suitable(ULONG family, IP_ADAPTER_ADDRESSES* aa)
{
    ...
    if (AF_INET == family) {
        for (IP_ADAPTER_GATEWAY_ADDRESS* gw = aa->FirstGatewayAddress;
            ...
            if (gw->Address.lpSockaddr &&
                ...
                if (is_valid_address(AF_INET, bytes)) return true;
            }
        }

        /* Accept physical Ethernet or Wi-Fi adapters with valid unicast
         * address. This handles WSL2, Hyper-V, Docker scenarios where gateway
         * info may be missing but the adapter is still functional for local
         * DNS.
         *
    ...
    for (IP_ADAPTER_UNICAST_ADDRESS* ua = aa->FirstUnicastAddress; ua != NULL;
        ...
        if (!ua->Address.lpSockaddr ||
            ua->Address.lpSockaddr->sa_family != family)
            continue;
        ...
        if (is_valid_address(family, bytes)) return true;
    }
 ...
 */
static IP_ADAPTER_ADDRESSES* get_adapter_addresses(pubnub_t* pb, ULONG family)
{
    ...
    do {
        ...
        if (!addrs) {
            PUBNUB_LOG_ERROR(
                pb,
                "Failed to allocate %lu bytes for adapter info (%s)",
        ...
        if (ret != ERROR_BUFFER_OVERFLOW ||
            ...
            PUBNUB_LOG_ERROR(
                pb,
                "GetAdaptersAddresses(%s) failed with error code: %lu",
                family == AF_INET ? "IPv4" : "IPv6",
                (unsigned long)ret);
            return NULL;
        }
    ...
    DWORD         if_index)
{
    ...
    if (sock == INVALID_SOCKET) {
        PUBNUB_LOG_ERROR(
            pb,
            "Validation socket creation failed with error code: %d",
            WSAGetLastError());
        return false;
    }
    ...
    if (setsockopt(
        ...
        PUBNUB_LOG_WARNING(
            pb,
            "Validation socket timeout configuration failed with error code: "
            "%d",
            WSAGetLastError());
    }
    ...
    if (addr_family == AF_INET) {
        ...
        PUBNUB_LOG_TRACE(
            pb,
            "Validating IPv4 DNS server: %u.%u.%u.%u...",
            dns_addr[0],
            dns_addr[1],
            dns_addr[2],
            dns_addr[3]);
        ...
        PUBNUB_LOG_TRACE(
            pb,
            "Validating IPv6 DNS server: %02x%02x:%02x%02x:%02x%02x:%02x%02x:"
        ...
        PUBNUB_LOG_ERROR(pb, "Unexpected address family: %d", addr_family);
        socket_close(sock);
        return false;
    }
    ...
    if (sent <= 0) {
        PUBNUB_LOG_DEBUG(
            pb,
            "sendto() for DNS validation failed with error code: %d",
            WSAGetLastError());
        socket_close(sock);
        return false;
    }
 ...
 */
static DWORD get_best_interface_index(pubnub_t* pb, ADDRESS_FAMILY family)
{
    ...
    if (family == AF_INET) {
        ...
        if (inet_pton(
                AF_INET6, "2001:4860:4860::8888", &dest_addr.Ipv6.sin6_addr) !=
            1)
            return 0;
    }
#endif
    ...
    DWORD              ret =
        GetBestRoute2(NULL, 0, NULL, &dest_addr, 0, &route, &source_addr);

    if (ret != NO_ERROR) {
        PUBNUB_LOG_DEBUG(
            pb,
            "GetBestRoute2(%s) failed with error code: %lu",
            family == AF_INET ? "IPv4" : "IPv6",
            (unsigned long)ret);
        return 0;
    }
    ...
    if (ret != NO_ERROR) {
        PUBNUB_LOG_DEBUG(
            pb,
            "ConvertInterfaceLuidToIndex(%s) failed with error code: "
            "%lu",
            family == AF_INET ? "IPv4" : "IPv6",
            (unsigned long)ret);
        return 0;
 ...
 */
struct adapter_info* pubnub_dns_read_system_servers(
    pubnub_t*             pb,
    ULONG                 family,
    ...
    size_t*               count)
{
    ...
    if (adapter_count == 0) {
        PUBNUB_LOG_WARNING(
            pb,
            "No suitable adapters found (%s)",
            family == AF_INET ? "IPv4" : "IPv6");
        *count = 0;
        return NULL;
    }

    struct adapter_info* adapter_list = (struct adapter_info*)MALLOC(
        adapter_count * sizeof(struct adapter_info));
    if (!adapter_list) {
        *count = 0;
        return NULL;
    }

    DWORD  best_if_index = get_best_interface_index(pb, family);
    size_t idx           = 0;
    for (IP_ADAPTER_ADDRESSES* aa = addrs; aa != NULL; aa = aa->Next) {
    ...
    for (size_t i = 0; i < adapter_count && total_dns_count < n; i++) {
        ...
        PUBNUB_LOG_TRACE(
            pb,
            "Processing adapter idx=%lu metric=%lu best=%d (%s)",
        ...
        for (IP_ADAPTER_DNS_SERVER_ADDRESS* ds = aa->FirstDnsServerAddress;
            ...
            if (AF_INET == family && dns_family != family) continue;
            ...
            for (size_t j = 0; j <= i; j++) {
                for (size_t k = 0; k < adapter_list[j].dns_count; k++) {
                    ...
                    if (AF_INET == dns_family && stored->ss_family == AF_INET) {
                        struct sockaddr_in* stored_sin =
                            (struct sockaddr_in*)stored;
                        if (memcmp(
                                &stored_sin->sin_addr.s_addr, dns_bytes, 4) ==
                            0) {
                            is_duplicate = true;
                            break;
                        }
                        ...
                        if (memcmp(&stored_sin6->sin6_addr, dns_bytes, 16) ==
                            0) {
                            is_duplicate = true;
                            break;
                        }
 ...
 *
 * @param pb PubNub context for logging.
    ...
    size_t                      n)
{
    ...
    if (!o_ipv4 || n == 0) return -1;

    IP_ADAPTER_ADDRESSES* addrs = get_adapter_addresses(pb, AF_INET);
    if (!addrs) return -1;

    size_t               count = 0;
    struct adapter_info* adapter_list =
        pubnub_dns_read_system_servers(pb, AF_INET, addrs, n, &count);
    if (!adapter_list || 0 == count) {
        FREE(addrs);
        if (adapter_list) FREE(adapter_list);
        return -1;
    }

    unsigned dns_count = 0;
    for (size_t i = 0; i < count; i++) {
        if (!adapter_list[i].dns_count) continue;
    ...
    if (dns_count == 0) {
        PUBNUB_LOG_ERROR(
            pb, "There are no valid DNS servers that have been found.");
        return -1;
    }

    PUBNUB_LOG_DEBUG(pb, "Discovered %u DNS server(s)", dns_count);
    ...
    size_t                      n)
{
    ...
    if (!o_ipv6 || n == 0) return -1;

    IP_ADAPTER_ADDRESSES* addrs = get_adapter_addresses(pb, AF_INET6);
    if (!addrs) return -1;

    size_t               count = 0;
    struct adapter_info* adapter_list =
        pubnub_dns_read_system_servers(pb, AF_INET6, addrs, n, &count);
    if (!adapter_list || 0 == count) {
        FREE(addrs);
        if (adapter_list) FREE(adapter_list);
        return -1;
    ...
    if (dns_count == 0) {
        PUBNUB_LOG_WARNING(
            pb, "There are no IPv6 DNS servers that have been found.");
        return 0;
    }

    PUBNUB_LOG_DEBUG(
        pb,
        "Discovered %u %s DNS server(s)",
        dns_count,
