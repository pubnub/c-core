/* -*- c-file-style:"stroustrup"; indent-tabs-mode: nil -*- */
    ...
    #define expect(exp) \
        if (exp) { \
            ...
            printf( \
                "\x1b[33m expect(%s) failed, file %s function %s line " \
                "%d\x1b[0m\n", \
                #exp, \
                __FILE__, \
                __FUNCTION__, \
    ...
    #define expect_pnr(rslt, exp_rslt) \
        do { \
            ...
            if (M_res_ != (exp_rslt)) { \
                printf( \
                    "\x1b[33m Expected result %d (%s) but got %d (%s), file %s " \
    ...
    #define ELAPSED_MS(prev_timspec) \
        do { \
            ...
            M_ms_diff = \
                (M_timspec.tv_nsec - (prev_timspec).tv_nsec) / MILLI_IN_NANO; \
            PUBNUB_LOG_TRACE( \
