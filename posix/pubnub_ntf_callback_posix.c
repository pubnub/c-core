        ...
        if (PUBNUB_TIMERS_API) {
            ...
            if (elapsed > 0) {
                if (elapsed > max_poll_ms + 5) {
                    PUBNUB_LOG_TRACE(
                        "elapsed = %d: prev_timspec={%ld, %ld}, timspec={%ld,%ld}\n",
                        elapsed,
                        prev_timspec.tv_sec,
                        prev_timspec.tv_nsec,
                        timspec.tv_sec,
                        timspec.tv_nsec
...
int pbntf_init(void) {
    ...
    if (rslt != 0) {
        PUBNUB_LOG_ERROR(
            "Failed to initialize mutex attributes, error code: %d",
            rslt);
        return -1;
    }
    ...
    if (rslt != 0) {
        PUBNUB_LOG_ERROR(
            "Failed to set mutex attribute type, error code: %d",
            rslt);
    ...
    if (rslt != 0) {
        PUBNUB_LOG_ERROR(
            "Failed to initialize 'stoplock' mutex, error code: %d",
            rslt);
        pthread_mutexattr_destroy(&attr);
        return -1;
    }
    ...
    if (rslt != 0) {
        PUBNUB_LOG_ERROR(
            "Failed to initialize mutex for timers, error code: %d",
            rslt);
        pthread_mutexattr_destroy(&attr);
